...
export function createSecurityReportHandler(options = {}) {
  ...
  return async function securityReportHandler(req, res) {
    ...
    const isGetSubmission = req.method === "GET" && req.query?.action === "submit";
    if (req.method !== "POST" && !isGetSubmission) {
    ...
    let body;
    if (isGetSubmission) {
      // Compatibility path for mobile executors whose generic request API is
      // blocked upstream while game:HttpGet remains available. Never accept a
      // raw HWID in the URL; it would otherwise be retained in access logs.
      body = {
    ...
    if (!webhookUrl || typeof fetchImpl !== "function") {
      console.warn("[Security Report] Webhook not configured; report was not delivered");
      return res.status(503).json({ error: "WEBHOOK_NOT_CONFIGURED", delivered: false });
    }
    ...
    try {
      ...
      if (!response?.ok) {
        const discordStatus = Number(response?.status) || 502;
        console.error(`[Security Report] Discord rejected report with status ${discordStatus}`);
      ...
      return res.status(502).json({ error: "DISCORD_DELIVERY_FAILED", delivered: false });
    }
