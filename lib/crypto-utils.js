...
function generateHoneypot() {
  ...
  return {
    [`__debug_${trapId}`]: {
      decryption_key: crypto.randomBytes(16).toString('hex'),
      bypass_token: crypto.randomBytes(32).toString('hex'),
      _warning: "TRAP_DETECTED"
    },
    [`__trap_${trapId}`]: `TRAP_${Date.now()}_${crypto.randomBytes(8).toString('hex')}`
  };
}
...
function isHoneypotTriggered(requestData) {
  ...
  if (requestString.includes('__debug_') || 
      requestString.includes('__trap_') ||
      requestString.includes('TRAP_DETECTED') ||
      requestString.includes('bypass_token')) {
    return true;
  }
