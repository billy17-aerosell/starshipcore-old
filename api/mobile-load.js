...
export default async function handler(req, res) {
  ...
  try {
      ...
      if (mobileUser.status === 'active') {
        ...
        if (mobileUser.expiresAt) {
          ...
          if (expiryTimestamp < now) {
            console.log(`[${timestamp}] ❌ Mobile VIP expired - UserID: ${userId}`);
        ...
        console.log(`[${timestamp}] 📱 MOBILE VIP ACCESS - UserID: ${userId} (${mobileUser.username}) | IP: ${clientIP}`);

        // Calculate remaining days
        let remainingDays = null;
        if (mobileUser.expiresAt) {
          const expiryTimestamp = Math.floor(new Date(mobileUser.expiresAt).getTime() / 1000);
          ...
          try {
            ...
            if (!existingDevice) {
              ...
              if (devices.length >= maxDevices) {
                console.log(`[${timestamp}] ❌ Mobile device limit reached - UserID: ${userId} (${devices.length}/${maxDevices})`);
              ...
              await redisClient.set(deviceKey, JSON.stringify(devices), 'EX', 86400 * 30); // 30 days
              ...
              await redisClient.set(deviceKey, JSON.stringify(devices), 'EX', 86400 * 30);
            }
        ...
        return res.status(200).json({
          ...
          expiry: mobileUser.expiresAt ? Math.floor(new Date(mobileUser.expiresAt).getTime() / 1000) : null,
          remainingDays: remainingDays,
          activatedAt: mobileUser.addedAt ? Math.floor(new Date(mobileUser.addedAt).getTime() / 1000) : null,
        ...
        console.log(`[${timestamp}] 🚫 SUSPENDED Mobile VIP - UserID: ${userId}`);
    ...
    if (fs.existsSync(mobileKeysPath)) {
      try {
        ...
        if (fileWhitelist[userId]) {
          ...
          let remainingDays = null;
          if (userData.expiresAt) {
            const expiryTimestamp = Math.floor(new Date(userData.expiresAt).getTime() / 1000);
            remainingDays = Math.ceil((expiryTimestamp - now) / 86400);
          }

          console.log(`[${timestamp}] 📱 MOBILE ACCESS (File) - UserID: ${userId} (${userData.username}) | IP: ${clientIP}`);
          ...
          return res.status(200).json({
            ...
            expiry: userData.expiresAt ? Math.floor(new Date(userData.expiresAt).getTime() / 1000) : null,
            remainingDays: remainingDays,
    ...
    // === NOT WHITELISTED ===
    console.log(`[${timestamp}] ❌ NOT MOBILE WHITELISTED - UserID: ${userId} | IP: ${clientIP}`);
