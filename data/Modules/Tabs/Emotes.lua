local function SetupEmotesUI(ScreenGui, UI, Connections, Config, LocalPlayer, UIHandlers, RegisterTheme)
	...
	local function LoadTrack(id)
		...
		if Settings["Stop Other Animations On Play"] then
			for _, t in pairs(humanoid.Animator:GetPlayingAnimationTracks()) do
				if t ~= newTrack and t.Priority ~= Enum.AnimationPriority.Action4 then
					t:Stop()
				end
			end
		end
	...
	local function RefreshSaved()
		for _, c in pairs(ContentSaved:GetChildren()) do
			if c:IsA("Frame") then
				c:Destroy()
			end
		end
	...
	BtnSaved.MouseButton1Click:Connect(function()
		ContentCatalog.Visible = false
		ContentSaved.Visible = true
		BtnCatalog.BackgroundColor3 = C_ITEM
		BtnCatalog.TextColor3 = C_TEXT_DIM
		BtnSaved.BackgroundColor3 = C_ACCENT
		BtnSaved.TextColor3 = Color3.new(0, 0, 0)
		RefreshSaved()
	end)
	...
	local function AddToggle(name)
		...
		b.MouseButton1Click:Connect(function()
			...
			b.TextColor3 = Settings[name] and C_GREEN or C_RED
			ts.Color = Settings[name] and C_GREEN or C_RED
			if name == "Looped" and CurrentTrack then
				CurrentTrack.Looped = Settings[name]
			end
		end)
	end
	...
	local function AddSlider(name, min, max)
		...
		UserInputService.InputChanged:Connect(function(i)
			if dragging and i.UserInputType == Enum.UserInputType.MouseMovement then
				...
				l.Text = name .. ": " .. Settings[name]
				if name == "Speed" and CurrentTrack then
					CurrentTrack:AdjustSpeed(Settings[name])
				end
			end
		end)
	end
