local function SetupFunUI(PageFun, UI, Connections, Config, LocalPlayer, UIHandlers, RegisterTheme)
    ...
    BtnAura.MouseButton1Click:Connect(function()
        ...
        -- Function to collect a part (teleport to player for ownership)
        local function CollectPart(part)
            if collectedParts[part] then return false end

            -- Store original position
            local origCFrame = part.CFrame

            pcall(function()
                -- Teleport to near player to claim ownership
                part.CFrame = myRoot.CFrame * CFrame.new(0, 10, 0) -- Above player

                -- Make it heavy and bouncy for fling
                part.CanCollide = true
        ...
        local function LaunchPart(part, targetPos)
            ...
            if not data or data.state == "launched" then return end
        ...
        auraLoop = RunService.Heartbeat:Connect(function()
            ...
            -- Launch collected parts one by one (or in bursts)
            local launchedThisFrame = 0
            local maxLaunchPerFrame = 3

            for part, data in pairs(collectedParts) do
                if data.state == "collected" and launchedThisFrame < maxLaunchPerFrame then
                    -- Wait a bit after collection before launching
                    if tick() - data.collectTime > 0.2 then
            ...
            AuraStatus.Text = "🚀 Active: " ..
                activeCount .. " | Launched: " .. launchedCount .. " | Dist: " .. distToTarget
        end)
    end)
