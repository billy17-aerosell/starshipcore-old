local function SetupFunUI(PageFun, UI, Connections, Config, LocalPlayer, UIHandlers, RegisterTheme)
	...
	BtnAura.MouseButton1Click:Connect(function()
			...
			for _, data in pairs(collectedParts) do
				if data.part and data.part.Parent then
					pcall(function()
						...
						local bp = data.part:FindFirstChild("FlingPosition")
						if bp then
							bp:Destroy()
						end
		...
		warn("[Fling] Starting DIRECT FLING attack on: " .. selectedPlayer4.Name)
		...
		-- Function to collect a part (orbit around player for visibility)
		local function CollectPart(part)
			...
			-- Assign unique orbit angle for this part
			local partCount = 0
			for _ in pairs(collectedParts) do
				partCount = partCount + 1
			end
			local orbitAngle = (partCount * math.pi * 2 / 8) + math.random() * 0.5 -- Spread evenly
			local orbitHeight = -2 + (partCount % 5) * 1.2 -- Multiple layers
			local orbitRadius = 4 + (partCount % 3) * 1.5 -- Varying radius

			pcall(function()
				-- DISABLE collision while orbiting (so you don't fling yourself!)
				part.CanCollide = false
				part.CustomPhysicalProperties = PhysicalProperties.new(50, 0.3, 1.5, 100, 100)

				-- Try to claim network ownership
				part:SetNetworkOwner(LocalPlayer)

				-- Initial position in orbit (visible to everyone!)
				local orbitPos = myRoot.Position
					+ Vector3.new(math.cos(orbitAngle) * orbitRadius, orbitHeight, math.sin(orbitAngle) * orbitRadius)
				part.CFrame = CFrame.new(orbitPos)
			...
			collectedParts[part] = {
				...
				state = "orbiting", -- Changed from "collected" to "orbiting"
				collectTime = tick(),
				orbitAngle = orbitAngle,
				orbitRadius = orbitRadius,
				orbitHeight = orbitHeight,
				orbitSpeed = 2 + math.random() * 1, -- Speed variation
			}

			return true
		end

		-- Function to launch a part at target
		local function LaunchPart(part, targetPos)
			local data = collectedParts[part]
			if not data or data.state == "launched" then
				return
			end

			pcall(function()
				if auraInstantTeleport then
					-- INSTANT TELEPORT MODE: Teleport directly to target and orbit
					local t = tick()
					local angle = t * 10 + math.random() * math.pi * 2
					local radius = 3 + math.random() * 2
					local orbitPos = targetPos
						+ Vector3.new(math.cos(angle) * radius, math.sin(t * 8) * 2 + 1, math.sin(angle) * radius)

					-- Teleport to orbit position
					part.CFrame = CFrame.new(orbitPos)

					-- ENABLE collision now (attacking target!)
					part.CanCollide = true

					-- High velocity towards target center for impact
					local dirToCenter = (targetPos - orbitPos).Unit
					part.AssemblyLinearVelocity = dirToCenter * 150
					part.AssemblyAngularVelocity =
						Vector3.new(math.random(-100, 100), math.random(-100, 100), math.random(-100, 100))

					data.state = "launched"
					data.launchTime = tick()

					-- Re-teleport rapidly for continuous attack
					task.spawn(function()
						while isAuraActive and data.state == "launched" and part and part.Parent do
							task.wait(0.1) -- Rapid re-teleport
							...
							if not targetRoot2 then
								break
							end

							local newTargetPos = targetRoot2.Position
							local newAngle = tick() * 15 + math.random() * math.pi
							local newRadius = 2 + math.random() * 2
							local newOrbitPos = newTargetPos
								+ Vector3.new(
							...
							pcall(function()
								part.CFrame = CFrame.new(newOrbitPos)
								local dir = (newTargetPos - newOrbitPos).Unit
								part.AssemblyLinearVelocity = dir * 200
								part.AssemblyAngularVelocity =
									Vector3.new(math.random(-150, 150), math.random(-150, 150), math.random(-150, 150))
								part.CanCollide = true
								sethiddenproperty(LocalPlayer, "SimulationRadius", math.huge)
							end)
						end
					...
					-- Remove any existing velocity constraint
					local oldBv = part:FindFirstChild("FlingVelocity")
					if oldBv then
						oldBv:Destroy()
					end

					-- Add BodyVelocity for consistent launch
					local bv = Instance.new("BodyVelocity", part)
					bv.Name = "FlingVelocity"
					bv.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
					bv.P = 10000
					bv.Velocity = dir * auraLaunchPower

					-- ENABLE collision now (attacking target!)
					part.CanCollide = true

					-- Also set direct velocity
					part.AssemblyLinearVelocity = dir * auraLaunchPower

					-- Add spin for more chaos
					part.AssemblyAngularVelocity =
					...
					-- Remove BodyVelocity after a short time so physics takes over
		...
		for _, part in pairs(workspace:GetDescendants()) do
			...
			then
				-- Check distance to PLAYER (not target) for easier collection
		...
		-- Also scan near target (will teleport to player)
		pcall(function()
			LocalPlayer:RequestStreamAroundAsync(targetRoot.Position, 10)
		end)

		task.wait(0.5) -- Wait for streaming
		...
		if totalCollected > 0 then
			AuraStatus.Text = "🎯 " .. totalCollected .. " objects collected! Launching..."
		...
		-- Main loop - continuously launch objects at target
		local launchIndex = 0
		...
		auraLoop = RunService.Heartbeat:Connect(function()
			...
			-- Update orbiting parts and launch when ready
			local launchedThisFrame = 0
			local maxLaunchPerFrame = 3
			local orbitingCount = 0
			local launchedCount = 0

			for part, data in pairs(collectedParts) do
				if data.state == "orbiting" then
					orbitingCount = orbitingCount + 1

					-- Keep parts orbiting around player (visible to everyone!)
					pcall(function()
						data.orbitAngle = data.orbitAngle + data.orbitSpeed * 0.016 -- ~60fps
						local orbitPos = myRoot2.Position
							+ Vector3.new(
								math.cos(data.orbitAngle) * data.orbitRadius,
								data.orbitHeight + math.sin(tick() * 3) * 0.5, -- Bobbing
					...
					-- After orbiting for 2 seconds, launch at target!
					if tick() - data.collectTime > 2 and launchedThisFrame < maxLaunchPerFrame then
						LaunchPart(part, targetPos)
						launchedThisFrame = launchedThisFrame + 1
					end
				elseif data.state == "launched" then
					launchedCount = launchedCount + 1
					-- Re-launch if part slowed down and didn't hit target yet
					local distToTarget = (part.Position - targetPos).Magnitude
					local vel = part.AssemblyLinearVelocity.Magnitude

					if distToTarget > 10 and vel < 50 and tick() - data.launchTime > 0.5 then
						-- Part slowed down, collect it back for re-orbit
						data.state = "orbiting"
			...
			if orbitingCount > 0 or launchedCount > 0 then
				AuraStatus.Text = "⭕ " .. orbitingCount .. " orbiting | 🎯 " .. launchedCount .. " attacking"
				AuraStatus.TextColor3 = C_GREEN
			end
			...
			if tick() - lastScanTime > 3 then
				...
				for _, part in pairs(workspace:GetDescendants()) do
					if
						...
						and not collectedParts[part]
					then
						-- Check both near player and near target
			...
			for _, data in pairs(collectedParts) do
				if data.part and data.part.Parent then
					...
					if data.state == "launched" then
						launchedCount = launchedCount + 1
					end
				end
			end

			local distToTarget = math.floor((myRoot2.Position - targetPos).Magnitude)
			AuraStatus.Text = "🚀 Active: "
				.. activeCount
				.. " | Launched: "
				.. launchedCount
				.. " | Dist: "
