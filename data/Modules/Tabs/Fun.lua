local function SetupFunUI(PageFun, UI, Connections, Config, LocalPlayer, UIHandlers, RegisterTheme)
	...
	local CardAura = CreateCard("OBJECT AURA (TRASH THROWER)", 280, 7)
	...
	local auraCollectionRange = 30
	local auraPowerRadius = 5

	local SliderCollection = CreateSlider(CardAura, "Collection Range", 10, 100, 30, function(val)
		auraCollectionRange = val
	end)
	SliderCollection.Position = UDim2.new(0.03, 0, 0, 115)

	local SliderPower = CreateSlider(CardAura, "Power Radius", 2, 20, 5, function(val)
		auraPowerRadius = val
	end)
	SliderPower.Position = UDim2.new(0.03, 0, 0, 160)

	local BtnAura = Instance.new("TextButton", CardAura)
	BtnAura.Text = "START AURA"
	BtnAura.Size = UDim2.new(0.94, 0, 0, 30)
	BtnAura.Position = UDim2.new(0.03, 0, 0, 210) -- Moved down
	...
	AuraStatus.Position = UDim2.new(0, 0, 0, 245) -- Moved down
	...
	BtnAura.MouseButton1Click:Connect(function()
		...
		-- Start Aura
		isAuraActive = true
		BtnAura.Text = "STOP AURA"
		BtnAura.BackgroundColor3 = C_RED
		AuraStatus.Text = "Attacking " .. selectedPlayer4.Name .. "!"
		...
		if myRoot then
			for _, part in pairs(workspace:GetDescendants()) do
				...
				then
					if (part.Position - myRoot.Position).Magnitude < auraCollectionRange then
		...
		if #controlledParts == 0 then
			AuraStatus.Text = "No unanchored parts nearby!"
			...
			task.delay(2, function()
				if isAuraActive then
					if #controlledParts > 0 then
						...
						AuraStatus.Text = "Active (No parts nearby)"
						AuraStatus.TextColor3 = C_YELLOW
					end
				end
			end)
				...
				if myRoot then
					for _, part in pairs(workspace:GetDescendants()) do
						if
							...
							and not part:IsDescendantOf(LocalPlayer.Character)
						then
							if (part.Position - myRoot.Position).Magnitude < auraCollectionRange then -- Collect nearby
								local alreadyControlled = false
								for _, p in pairs(controlledParts) do
									if p == part then
										alreadyControlled = true
										break
									end
								end
								if not alreadyControlled then
									table.insert(controlledParts, part)
									pcall(function()
										part:SetNetworkOwner(LocalPlayer)
										part.CanCollide = false -- Disable collision for smoother movement initially
									end)
								end
							end
						end
		...
		auraLoop = RunService.Heartbeat:Connect(function()
			...
			local t = tick()
			for i, part in pairs(controlledParts) do
				if part and part.Parent then
					-- AGGRESSIVE ORBIT
					-- Faster rotation (t * 10)
					-- Tighter radius (5 + ...)
					local angle = (t * 10) + (i * (math.pi * 2 / #controlledParts))
					local radius = auraPowerRadius + math.sin(t * 5) * (auraPowerRadius * 0.5) -- Fluctuates based on power

					-- "Slam" effect: Occasionally pull directly to center
					if math.sin(t * 2) > 0.8 then
						radius = 0.5
					end

					local targetPos = targetRoot.Position
						+ Vector3.new(math.cos(angle) * radius, math.sin(t * 8 + i) * 3, math.sin(angle) * radius)
					local diff = targetPos - part.Position

					-- Physics move (Aggressive)
					part.AssemblyLinearVelocity = diff * 60 -- SUPER FAST PULL
					part.AssemblyAngularVelocity = Vector3.new(50, 100, 50) -- CRAZY SPIN

					-- Force ownership & properties
					pcall(function()
						part:SetNetworkOwner(LocalPlayer)
						part.CanCollide = true -- Re-enable collision to hit player
					end)

					-- Teleport if stuck/far (Tighter threshold)
					if diff.Magnitude > 40 then
						part.CFrame = CFrame.new(targetPos)
						part.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
					end
				else
					table.remove(controlledParts, i)
				end
			end
		end)
	end)
