...
local C_MAIN=Color3.fromRGB(25,25,30); local C_SIDE=Color3.fromRGB(20,20,25); local C_ACCENT=Color3.fromRGB(0,200,255); local C_TEXT=Color3.fromRGB(240,240,240); local C_TEXT_DIM=Color3.fromRGB(150,150,150); local C_ITEM=Color3.fromRGB(40,40,45); local C_RED=Color3.fromRGB(255,60,60); local C_YELLOW=Color3.fromRGB(255,200,0); local C_GREEN=Color3.fromRGB(0,255,150)
...
function Intro.StartLoader(Main, TargetMainHeight)
    local success, err = pcall(function()
        ...
        -- 3. Subtitle Slide Up
        TweenService:Create(Sub, TweenInfo.new(0.8, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {TextTransparency = 0, Position = UDim2.new(0,0,0.52,0)}):Play()
        task.wait(1.5)
        ...
        -- 5. Exit Animation
        TweenService:Create(Title, TweenInfo.new(0.5), {TextTransparency = 1, Position = UDim2.new(0,0,0.4,0)}):Play()
        TweenService:Create(Sub, TweenInfo.new(0.5), {TextTransparency = 1, Position = UDim2.new(0,0,0.6,0)}):Play()
        ...
        if Main then
            ...
            -- Enable Blur
            if Main:FindFirstChild("MainBlur") then TweenService:Create(Main.MainBlur, TweenInfo.new(1.0), {Size = 24}):Play() end
            
            local tw = TweenService:Create(Main, TweenInfo.new(1.0, Enum.EasingStyle.Exponential, Enum.EasingDirection.Out), {Size = targetSize})
            tw:Play()
        end
