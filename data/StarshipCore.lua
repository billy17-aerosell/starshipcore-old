...
do
	...
	RegisterDynamicUI(BtnStart, function(el)
		el.Text = "▶  " .. L("start")
	end)
	RegisterDynamicUI(BtnPause, function(el)
		el.Text = "⏸  " .. L("pause")
	end)
...
function UIHandlers.InitMergerUI()
	...
	RegisterDynamicUI(MergerRefs.SearchBox, function(el)
		el.PlaceholderText = L("search_files")
	end)
			...
			task.spawn(function()
				...
				if S.isAutoCloudSync then
					...
					if getgenv and getgenv().StarshipSession then
						...
						cloudEnabled = (role:upper() == "OWNER" or role:upper() == "DEVELOPER")
					end
					...
					if cloudEnabled then
						task.spawn(function()
							...
							if CloudRecording then
								ShowToast("☁️ Syncing...", "Uploading to cloud...", "info", 2)

								CloudRecording.SaveOrUpdate(finalData, name, {
									onDuplicate = function(existing, saveNew, updateExisting)
										-- Auto-update without confirmation
										updateExisting()
									end,
									onSuccess = function(result)
										local actionText = result.action == "updated" and "Updated!" or "Uploaded!"
										ShowToast("☁️ " .. actionText, "Synced to cloud!", "success", 2)
									end,
									onError = function(err)
										ShowToast("☁️ Sync Failed", err or "Upload error", "error", 3)
									end,
								})
							end
						end)
...
function UIHandlers.SetupListMapUI()
	...
	RegisterDynamicUI(SearchBar, function(el)
		el.PlaceholderText = L("search_files")
	end)
		...
		if isCloudEnabled then
			...
			BtnCloud.MouseButton1Click:Connect(function()
				...
				if not CloudRecording then
					ShowToast("Error", "Cloud module not available", "error", 2)
					return
				end

				-- Show loading state
				BtnCloud.Text = "⏳"
				BtnCloud.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
				...
				if not success then
					BtnCloud.Text = "☁️"
					BtnCloud.BackgroundColor3 = Color3.fromRGB(59, 130, 246)
					ShowToast("Error", "Failed to read file", "error", 2)
					return
				end

				local dataSuccess, recordingData = pcall(function()
					return HttpService:JSONDecode(content)
				end)

				if not dataSuccess or not recordingData then
					BtnCloud.Text = "☁️"
					BtnCloud.BackgroundColor3 = Color3.fromRGB(59, 130, 246)
					ShowToast("Error", "Invalid recording data", "error", 2)
					return
				end

				-- Upload to cloud with smart duplicate detection
				local recordingName = fileName:gsub(".json", "")

				-- Use SaveOrUpdate for smart duplicate handling
				CloudRecording.SaveOrUpdate(recordingData, recordingName, {
					onDuplicate = function(existingRec, saveNew, updateExisting)
						-- Reset button while waiting for user decision
						BtnCloud.Text = "☁️"
						BtnCloud.BackgroundColor3 = Color3.fromRGB(59, 130, 246)
						...
						if ShowConfirm then
							ShowConfirm(
								...
								"'"
									.. recordingName
									.. "' already exists in cloud!\n\nClick YES to UPDATE the existing recording.\nClick NO to cancel.",
								function()
							...
							-- No ShowConfirm available, default to update
							updateExisting()
						end
					end,
					...
					onError = function(errorMsg)
						BtnCloud.Text = "❌"
						BtnCloud.BackgroundColor3 = C_RED
						ShowToast("Error", errorMsg or "Upload failed", "error", 3)
	...
	UIHandlers.RefreshListMap = RefreshMapList -- Expose for cloud download
