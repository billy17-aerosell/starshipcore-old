...
if getgenv then getgenv().StarshipRunning = false end -- Reset running state for debug
...
if not isfolder(WARP_FOLDER) then makefolder(WARP_FOLDER) end
if not isfolder(MERGER_FOLDER) then makefolder(MERGER_FOLDER) end
...
local C_MAIN, C_SIDE, C_ACCENT, C_TEXT, C_TEXT_DIM, C_ITEM, C_RED, C_YELLOW, C_GREEN = 
    CurrentColors.MAIN, CurrentColors.SIDE, CurrentColors.ACCENT, CurrentColors.TEXT, CurrentColors.TEXT_DIM, CurrentColors.ITEM, CurrentColors.RED, CurrentColors.YELLOW, CurrentColors.GREEN
    ...
    if name == "Config" or not _G.StarshipModulePrefix then
        ...
        local function scan(dir)
            ...
            for _, file in ipairs(files) do
                ...
                if not file:lower():match("%.lua$") and not file:lower():match("%.json$") and not file:lower():match("%.txt$") then
                    local res = scan(file)
...
if not UIModule then UIModule = {} end

local IntroModule = LoadModule("Intro")
if not IntroModule then IntroModule = {} end

local Themes = ConfigData.Themes or {}
local Config = ConfigData.DefaultConfig or {
    Theme = "Default",
...
local S = {
    isRecording = false, isPaused = false, isPlaying = false, isPlayPaused = false,
    isLooping = false, isGodMode = false, playbackSpeed = 1.0, isMoonwalk = false,
    isFlexibleRecording = false, isStrictRetarget = false, isNativeAnim = false, isWarpLoop = false,
    isReversing = false, isZoomPunch = false, lastAirState = nil, isRespawnOnEnd = false,
    isLiveSmoothing = false, liveSmoothingStrength = 3, isBinding = false,
    currentWorkspace = "Default", currentMergerWorkspace = "Default",
    GlobalKeyDuration = "N/A", TargetMainHeight = 380, startTime = 0
}
...
local isReversing, isZoomPunch, lastAirState, isRespawnOnEnd = S.isReversing, S.isZoomPunch, S.lastAirState, S.isRespawnOnEnd
local isFlexibleRecording, isStrictRetarget, isNativeAnim, isWarpLoop = S.isFlexibleRecording, S.isStrictRetarget, S.isNativeAnim, S.isWarpLoop
...
local currentPlaybackFile, currentPlaybackTime, currentTotalDuration, currentFrameData = Playback.file, Playback.time, Playback.totalDuration, Playback.frameData
        ...
        for i = 2, #processedFrames - 1 do
            ...
            if isFlexible then
                if prev.pos and curr.pos and next.pos then
                    ...
                    curr.pos.x = newPos.X; curr.pos.y = newPos.Y; curr.pos.z = newPos.Z
                end
                if prev.vel and curr.vel and next.vel then
                    ...
                    curr.vel.x = newVel.X; curr.vel.y = newVel.Y; curr.vel.z = newVel.Z
                end
            else
                if prev.r and curr.r and next.r then
                    local cf1 = TblToCF(prev.r); local cf2 = TblToCF(curr.r); local cf3 = TblToCF(next.r)
...
function UIHandlers.SetLiveSmoothing(enabled, strength)
    ...
    if strength then liveSmoothingStrength = strength end
end
...
function UIHandlers.SmoothRecording(strength)
    ...
    if isPathEnabled then GeneratePlaybackPath(recordedData.Frames) end
end
...
if not isfolder(RECORDER_FOLDER) then makefolder(RECORDER_FOLDER) end
...
local function CleanupConnections()
    ...
    if UIHandlers and UIHandlers.CleanupTools then 
        pcall(function() UIHandlers.CleanupTools() end) 
    end
end
...
local function StartToolMonitor()
    if toolMonitorConnection then return end
    toolMonitorConnection = RunService.Heartbeat:Connect(function()
        if not isAnyTextBoxFocused then return end
    ...
    local success, err = pcall(function()
            ...
            for i = 1, 30 do
                ...
                task.spawn(function()
                    ...
                    while star and star.Parent and not skipped do
                        ...
                        if newY < -0.1 then newY = 1.1; startY = 1.1 end
                        startY = newY
        ...
        local function DoSkip()
            ...
            if Main then
                ...
                local tw = TweenService:Create(Main, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = targetSize})
                tw:Play()
                tw.Completed:Wait()
        ...
        if Main then
            ...
            -- Unfold animasi
            local tw = TweenService:Create(Main, TweenInfo.new(0.8, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = targetSize})
            tw:Play()
            tw.Completed:Wait()
...
local FIREBASE_WHITELIST_URL = "https://starship-nametags-default-rtdb.asia-southeast1.firebasedatabase.app/whitelist"
local CurrentUserRole = "VIP" -- Default role

local function CheckLicense(_, userId)
    local url = FIREBASE_WHITELIST_URL .. "/" .. userId .. ".json"
    local success, response = pcall(function() return game:HttpGet(url) end)
    
    if not success then return false, "Connection Error", nil end
    
    if response == "null" or response == "" then
        return false, "User Not Whitelisted", nil
    end
    ...
    if data then
        ...
        elseif type(data) == "table" then
            ...
            return true, duration, role
        end
    end
    
    return false, "Invalid License Data", nil
end

local function KillSession(reason)
    CleanupConnections()
    if getgenv().ToggleNametags then getgenv().ToggleNametags(false) end
    if UIHandlers.CleanupSpeedDisplay then UIHandlers.CleanupSpeedDisplay() end
...
local function StartSessionValidation()
    task.spawn(function()
        while true do
            task.wait(300) -- Check every 5 minutes
            local isValid, msg = CheckLicense(nil, tostring(LocalPlayer.UserId))
            if not isValid then
                if msg == "Connection Error" then
                    ...
                    -- Show warning toast
                    if UIModule and UIModule.ShowToast then
                        UIModule.ShowToast("Connection Warning", "Failed to verify license (" .. SessionFailCount .. "/" .. MAX_FAIL_GRACE .. ")", "warning", 5)
                    end
                else
...
local function SetupKeySystem()
    ...
    local keyFrameWidth = Mobile.IsCompact and math.min(Mobile.ViewportSize.X - 40, 350) or 400
    local MainFrame = Instance.new("Frame", KeyGui)
    MainFrame.Size = UDim2.new(0, keyFrameWidth, 0, 0)
    MainFrame.Position = UDim2.new(0.5, -keyFrameWidth/2, 0.5, -100)
    ...
    task.spawn(function()
        ...
        while Title and Title.Parent do
            ...
            local c = Color3.fromHSV(t % 1, 0.8, 1)
            local r, g, b = math.floor(c.R*255), math.floor(c.G*255), math.floor(c.B*255)
            Title.Text = string.format('<font color="rgb(%d,%d,%d)">STARSHIP</font> <font color="rgb(255,255,255)" size="14">v0.9-BETA</font>', r, g, b)
            task.wait(0.03)
        end
    end)
    ...
    Welcome.BackgroundTransparency = 1; Welcome.TextTransparency = 1
    Welcome.TextColor3 = Color3.fromRGB(200, 200, 200)
    Welcome.Font = Enum.Font.GothamBold
    Welcome.TextSize = 14
    
    local StatusMsg = Instance.new("TextLabel", MainFrame)
    StatusMsg.Text = "Checking whitelist..."
    StatusMsg.Size = UDim2.new(1, 0, 0, 25)
    StatusMsg.Position = UDim2.new(0, 0, 0, 80)
    StatusMsg.BackgroundTransparency = 1; StatusMsg.TextTransparency = 1
    StatusMsg.TextColor3 = C_YELLOW
    StatusMsg.Font = Enum.Font.GothamBold
    StatusMsg.TextSize = 14

    local UserID = tostring(LocalPlayer.UserId)
    local IDLabel = Instance.new("TextButton", MainFrame)
    IDLabel.Text = "User ID: " .. UserID .. " (Click to copy)"
    IDLabel.Size = UDim2.new(0.8, 0, 0, 25)
    IDLabel.Position = UDim2.new(0.1, 0, 0, 115)
    IDLabel.BackgroundTransparency = 1; IDLabel.TextTransparency = 1
    ...
    DurationLabel.BackgroundTransparency = 1; DurationLabel.TextTransparency = 1
    DurationLabel.TextColor3 = C_ACCENT
    DurationLabel.Font = Enum.Font.GothamBold
    DurationLabel.TextSize = 12

    -- Animation In
    TweenService:Create(MainFrame, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size = UDim2.new(0, keyFrameWidth, 0, 200), BackgroundTransparency = 0}):Play()
    task.wait(0.3)
    ...
    TweenService:Create(StatusMsg, fadeInfo, {TextTransparency = 0}):Play()
    TweenService:Create(IDLabel, fadeInfo, {TextTransparency = 0}):Play()
    
    task.wait(0.5)
    
    -- Auto-check whitelist via Firebase
    task.spawn(function()
        local isValid, result, role = CheckLicense(nil, UserID)
        
        if not isValid and result == "Connection Error" then
            StatusMsg.Text = "Connection Failed! Retrying..."
            StatusMsg.TextColor3 = C_RED
            task.wait(2)
            isValid, result, role = CheckLicense(nil, UserID)
        end
        
        if isValid then
            StatusMsg.Text = "ACCESS GRANTED"
            StatusMsg.TextColor3 = Color3.fromRGB(46, 204, 113)
            GlobalKeyDuration = string.upper(result)
            local roleText = role and (" [" .. string.upper(role) .. "]") or ""
            DurationLabel.Text = "License: " .. GlobalKeyDuration .. roleText
            TweenService:Create(DurationLabel, fadeInfo, {TextTransparency = 0}):Play()
            
            StartSessionValidation()
            task.wait(1.5)
            
            -- Close UI
            TweenService:Create(MainFrame, TweenInfo.new(0.5), {BackgroundTransparency = 1, Size = UDim2.new(0, keyFrameWidth, 0, 0)}):Play()
            ...
            TweenService:Create(StatusMsg, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
            TweenService:Create(IDLabel, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
            TweenService:Create(DurationLabel, TweenInfo.new(0.3), {TextTransparency = 1}):Play()
            ...
            StatusMsg.Text = "NOT WHITELISTED"
            StatusMsg.TextColor3 = Color3.fromRGB(231, 76, 60)
            DurationLabel.Text = result or "Contact admin to get access"
...
-- Quick Auto-Login (Check Firebase whitelist by UserId)
local function TryAutoLogin()
    local isValid, duration = CheckLicense(nil, tostring(LocalPlayer.UserId))
    if isValid then
...
-- --- HELPER FUNCTIONS ---
local function CFToTbl(cf) return {cf:GetComponents()} end
local function TblToCF(t) return CFrame.new(unpack(t)) end
...
local function GetJoints(char) local j={}; for _,d in pairs(char:GetDescendants()) do if d:IsA("Motor6D") then table.insert(j,d) end end; return j end
local function ResetChar()
    ...
    if r then 
        ...
        if r:FindFirstChild("PlaybackAtt") then r.PlaybackAtt:Destroy() end
        if r:FindFirstChild("PlaybackAO") then r.PlaybackAO:Destroy() end
        if r:FindFirstChild("PlaybackAP") then r.PlaybackAP:Destroy() end
    end
...
-- --- PATH VISUALIZER ---
local function InitPathFolder() if not PathContainer or not PathContainer.Parent then PathContainer=Instance.new("Folder",workspace); PathContainer.Name="WalkGemPath" end end
local function ClearPath() if PathContainer then PathContainer:ClearAllChildren() end; lastPathPoint=nil end
local function DrawLine(p1, p2, color)
    InitPathFolder()
    local dist = (p1-p2).Magnitude; if dist<0.1 then return end
    local part=Instance.new("Part"); part.Anchored=true; part.CanCollide=false; part.Material=Enum.Material.Neon; part.Color=color or Color3.fromRGB(255,50,50); part.Size=Vector3.new(0.1,0.1,dist); part.CFrame=CFrame.lookAt(p1,p2)*CFrame.new(0,0,-dist/2); part.Parent=PathContainer
    return part
end
...
local function GeneratePlaybackPath(frames)
    ClearPath()
    if not isPathEnabled or not frames or #frames < 2 then return end
    task.spawn(function()
        local startFrame = frames[1]
        local prevPos = (startFrame.pos and Vector3.new(startFrame.pos.x, startFrame.pos.y, startFrame.pos.z)) or (startFrame.r and TblToCF(startFrame.r).Position)
        if not prevPos then return end
        
        for i = 2, #frames do
            if not isPlaying and not isPlayPaused then break end
            local f = frames[i]
...
local function StopRecording()
    ...
    if Connections.Record then Connections.Record:Disconnect() end
    if Connections.Preview then Connections.Preview:Disconnect() end
    ResetChar()
end
local function StartRecording()
    if isRecording or isPlaying then return end; local c=LocalPlayer.Character; if not c then return end
    
    -- CLEANUP PREVIOUS PATHS
    ClearPath()
    if PathContainer then PathContainer:ClearAllChildren() end 
    
    isRecording=true; isPaused=false; recordedData={FPS=60,Frames={}}; startTime=os.clock(); local joints=GetJoints(c); 
    CreateStartBaseplate(c.HumanoidRootPart.Position); lastPathPoint=c.HumanoidRootPart.Position
    
    Connections.Record = RunService.Heartbeat:Connect(function()
        if isPaused then return end; if not c.Parent then StopRecording() return end; local r=c:FindFirstChild("HumanoidRootPart"); local h=c:FindFirstChild("Humanoid"); if not r or not h then return end
        ...
        if isFlexibleRecording then
            ...
            fd.j = {}
            for _,m in ipairs(joints) do fd.j[m.Name]=CFToTbl(m.Transform) end
        ...
        if isPathEnabled then local cp=r.Position; if (cp-lastPathPoint).Magnitude>0.5 then DrawLine(lastPathPoint,cp,currentPathColor); lastPathPoint=cp end end
    end)
end
local function CutAndResume(cutTime)
    if Connections.Preview then Connections.Preview:Disconnect() end; if not recordedData.Frames or #recordedData.Frames==0 then return end
    local nf={}; for _,f in ipairs(recordedData.Frames) do if f.t<=cutTime then table.insert(nf,f) end end; recordedData.Frames=nf
    ClearPath()
    ...
    if isPathEnabled then 
        ...
        for _,f in ipairs(nf) do 
            local pos = (f.pos and Vector3.new(f.pos.x, f.pos.y, f.pos.z)) or (f.r and TblToCF(f.r).Position)
            if pos then
                if pp and (pos-pp).Magnitude>0.5 then DrawLine(pp,pos,Color3.fromRGB(255,50,50)); pp=pos elseif not pp then pp=pos end 
            end
        end
        if pp then lastPathPoint=pp end 
    end
    ...
    if lf then 
        ...
        if r then
            ...
            if lf.r then
                ...
                local j=GetJoints(c); if lf.j then for _,m in ipairs(j) do if lf.j[m.Name] then m.Transform=TblToCF(lf.j[m.Name]) end end end
            elseif lf.pos then
...
local function SaveRecording(fn) 
    if not recordedData.Frames or #recordedData.Frames==0 then return end
    ...
    if RefreshPlayerList then RefreshPlayerList() end -- Auto Refresh UI
    if UIHandlers.RefreshMergerList then UIHandlers.RefreshMergerList() end -- Auto Refresh Merge UI
end
-- ZOOM PUNCH EFFECT FUNCTION
local defaultFOV = 70
local currentZoomTween = nil
local function SetZoomState(state)
    if not isZoomPunch then return end
    local cam = workspace.CurrentCamera
    ...
    currentZoomTween = TweenService:Create(cam, TweenInfo.new(0.25, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {FieldOfView = targetFOV})
    currentZoomTween:Play()
end

local function StopPlayback()
    isPlaying=false; isPlayPaused=false; isReversing=false; lastAirState=nil; SetZoomState("normal"); if Connections.Playback then Connections.Playback:Disconnect() end; currentPlaybackTime=0; lastPlaybackTime=0
    ...
    playbackSpeed = 1.0
    if UIHandlers.BtnSpeed then UIHandlers.BtnSpeed.Text = "1x" end
...
local function PausePlayback() 
    if isPlaying then 
        isPlayPaused=true; isPlaying=false; if Connections.Playback then Connections.Playback:Disconnect() end 
        local c=LocalPlayer.Character
        if c then 
            local r=c:FindFirstChild("HumanoidRootPart"); 
            if r then 
                ...
                local ao = r:FindFirstChild("PlaybackAO"); if ao then ao:Destroy() end
                local ap = r:FindFirstChild("PlaybackAP"); if ap then ap:Destroy() end
                local att = r:FindFirstChild("PlaybackAtt"); if att then att:Destroy() end
            end
            local a=c:FindFirstChild("Animate"); if a then a.Disabled=false end
            local h=c:FindFirstChild("Humanoid"); if h then h.AutoRotate=true end
        end 
    end 
end
...
local function GetDistanceToNearestPathPoint(frames, playerPos)
    if not frames or #frames == 0 or not playerPos then return math.huge end
...
local function PlayRecording(fn, force, skipDistanceCheck)
    ...
    if not isfile(p) then
        ...
        p = MERGER_FOLDER.."/"..fn
        if not isfile(p) then ShowToast("Error", "File not found: " .. fn, "error", 3) return end
    end
    ...
    if currentPlaybackFile~=fn or force or not currentFrameData then 
        ...
        if not s then ShowLoadingModal(false) return end; 
            ...
            if r and #currentFrameData > 0 then
                ...
                if dist > MAP_DISTANCE_THRESHOLD then
                    ...
                    ShowToast(
                        ...
                        string.format("Nearest path point is %.0f studs away! This recording may be from a different game/map.", dist), 
                        "warning", 
                        5
                    )
    ...
    if not currentFrameData or #currentFrameData<2 then ShowLoadingModal(false) return end; 
    currentTotalDuration=currentFrameData[#currentFrameData].t
    local c=LocalPlayer.Character; local r=c and c:FindFirstChild("HumanoidRootPart"); 
    if not r then ShowLoadingModal(false) return end
    local h=c:FindFirstChild("Humanoid"); local a=c:FindFirstChild("Animate")
    ...
    if true then
        ...
        for i = 1, #currentFrameData, step do
            if i % 50 == 0 then task.wait() end -- Yield to prevent freeze
            
            local f = currentFrameData[i]
            local pos = (f.pos and Vector3.new(f.pos.x, f.pos.y, f.pos.z)) or (f.r and TblToCF(f.r).Position)
            if pos then
                local dst = (rPos - pos).Magnitude
                if dst < minDst then minDst = dst; bestT = f.t end
            end
        end
        ...
        local lastPos = (lastF.pos and Vector3.new(lastF.pos.x, lastF.pos.y, lastF.pos.z)) or (lastF.r and TblToCF(lastF.r).Position)
        if lastPos then
             local dst = (rPos - lastPos).Magnitude
             if dst < minDst then minDst = dst; bestT = lastF.t end
        end
    ...
    isPlaying=true; isPlayPaused=false; lastPlaybackTime=currentPlaybackTime
    ...
    local targetPos = (startFrame.pos and Vector3.new(startFrame.pos.x, startFrame.pos.y, startFrame.pos.z)) or (startFrame.r and TblToCF(startFrame.r).Position)
    ...
    if targetPos then
        if startFrame.r then CreateStartBaseplate(targetPos) end -- Only create baseplate if we have a position
        ...
        if dist > 3 then
            ...
            while isPlaying do
                ...
                if not isPlaying then h:MoveTo(r.Position); return end
                
                -- Refresh MoveTo every second
                if (os.clock() - moveStart) % 1 < 0.1 then h:MoveTo(targetPos) end
    ...
    if isFlexible then
        ...
        Connections.Playback = RunService.Stepped:Connect(function(_, dt)
            ...
            if not isPlaying or isPlayPaused then return end
            ...
            if fA and fB then
                ...
                elseif fA.vel and fB.vel then
                    ...
                    local vel = Vector3.new(fA.vel.x, fA.vel.y, fA.vel.z):Lerp(Vector3.new(fB.vel.x, fB.vel.y, fB.vel.z), alpha)
                ...
                elseif isClimbingOrSwimming then
                    ...
                    elseif fA.charLook then
                        ...
                        if fA.vel and fB.vel then
                            local v = Vector3.new(fA.vel.x, fA.vel.y, fA.vel.z):Lerp(Vector3.new(fB.vel.x, fB.vel.y, fB.vel.z), alpha)
                ...
                if fA.st then
                    ...
                    if stateName then
                        ...
                        if isReversing then
                            ...
                            local isInAir = (stateEnum == Enum.HumanoidStateType.Jumping or 
                                           stateEnum == Enum.HumanoidStateType.Freefall)
                            ...
                            local isAirState = (stateEnum == Enum.HumanoidStateType.Jumping or 
                                               stateEnum == Enum.HumanoidStateType.Freefall)
                ...
                if not skipDriftCorrection and fA.pos and fB.pos then
                    local targetPos = Vector3.new(fA.pos.x, fA.pos.y, fA.pos.z):Lerp(Vector3.new(fB.pos.x, fB.pos.y, fB.pos.z), alpha)
        ...
        if a then 
            ...
            if isNativeAnim and a.Disabled then a.Disabled = false end
        end
        
        -- Setup Constraints
        local att = r:FindFirstChild("PlaybackAtt") or Instance.new("Attachment", r); att.Name="PlaybackAtt"
        
        local ap = r:FindFirstChild("PlaybackAP") or Instance.new("AlignPosition", r); ap.Name="PlaybackAP"
        ap.Mode = Enum.PositionAlignmentMode.OneAttachment
        ap.Attachment0 = att
        ap.RigidityEnabled = true -- Infinite force to match Strict behavior
        
        local ao = r:FindFirstChild("PlaybackAO") or Instance.new("AlignOrientation", r); ao.Name="PlaybackAO"
        ao.Mode = Enum.OrientationAlignmentMode.OneAttachment
        ao.Attachment0 = att
        ao.RigidityEnabled = true -- Infinite torque
        
        local jm={}; for _,d in pairs(c:GetDescendants()) do if d:IsA("Motor6D") then jm[d.Name]=d end end
        ...
        local filterList = {c}
        if PathContainer then table.insert(filterList, PathContainer) end
            ...
            if fA and fB then
                ...
                if isNativeAnim or isStrictRetarget then
                     ...
                     if isStrictRetarget and groundSnapCalculated then
                         ...
                         finalPosition = Vector3.new(finalPosition.X, finalPosition.Y + groundSnapOffset, finalPosition.Z)
                     else
...
UIHandlers.RenameMergerFile = function(oldName, newName, callback)
    ...
    if not string.match(newName, "%.json$") then newName = newName .. ".json" end
...
local function SelectRecording(fn)
    ...
    local s,j=pcall(readfile,p); if not s then return end
    local d=HttpService:JSONDecode(j); currentFrameData=d.Frames or d; currentPlaybackFile=fn; currentPlaybackTime=0
    if not currentFrameData or #currentFrameData<2 then return end; currentTotalDuration=currentFrameData[#currentFrameData].t
    
    currentPlaybackSource = "Recorder" -- Set source flag
    
    if isPathEnabled then GeneratePlaybackPath(currentFrameData) else ClearPath() end
    
    local startFrame = currentFrameData[1]
    local targetPos = (startFrame.pos and Vector3.new(startFrame.pos.x, startFrame.pos.y, startFrame.pos.z)) or (startFrame.r and TblToCF(startFrame.r).Position)
    if targetPos then CreateStartBaseplate(targetPos) end
...
local function RenameFile(o,n) 
    if not n or n=="" then return end; if not string.find(n,".json") then n=n..".json" end
    local op,np=GetWorkspacePath().."/"..o,GetWorkspacePath().."/"..n; 
    if isfile(op) then writefile(np,readfile(op)); delfile(op) end 

end 
local function DeleteFile(f) 
    ShowConfirm("DELETE FILE", "Delete file '"..f.."' permanently?", function()
        local p=GetWorkspacePath().."/"..f; if isfile(p) then delfile(p) end 
...
local function SetTheme(c) 
    ...
    if SliderFill then SliderFill.BackgroundColor3=c end
    if PSliderFill then PSliderFill.BackgroundColor3=c end
    if CountLbl then CountLbl.TextColor3=c end
    if RewindTimeLbl then RewindTimeLbl.TextColor3=c end
    if BtnSave then BtnSave.BackgroundColor3=c end
    if ConfirmTitle then ConfirmTitle.TextColor3=c end
    if BtnYes then BtnYes.BackgroundColor3=c end
...
ScreenGui = Instance.new("ScreenGui"); ScreenGui.Name=GUI_NAME; ScreenGui.Parent=CoreGui; ScreenGui.IgnoreGuiInset = true
...
Main = Instance.new("Frame", ScreenGui); Main.Size=UDim2.new(0,Mobile.MAIN_WIDTH,0,380); Main.Position=UDim2.new(0.5,-Mobile.MAIN_WIDTH/2,0.5,-190); Main.BackgroundTransparency=1; Main.Active=true; Main.Visible=false 
...
MainBg.ZIndex = 0
local MainCorner = Instance.new("UICorner", MainBg); MainCorner.CornerRadius = UDim.new(0, 12)
...
ResizeHandle.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        if resizeCon then resizeCon:Disconnect() resizeCon = nil end
        if resizeEndCon then resizeEndCon:Disconnect() resizeEndCon = nil end
...
MinIcon.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        input.Changed:Connect(function() if input.UserInputState == Enum.UserInputState.End then UIHandlers.DragMin.dragging = false end end)
    end
end)
...
UserInputService.InputChanged:Connect(function(input) 
    if input == UIHandlers.DragMin.dragInput and UIHandlers.DragMin.dragging then 
        ...
        if delta.Magnitude > 5 then UIHandlers.DragMin.hasDragged = true end
        MinIcon.Position = UDim2.new(UIHandlers.DragMin.startPos.X.Scale, UIHandlers.DragMin.startPos.X.Offset + delta.X, UIHandlers.DragMin.startPos.Y.Scale, UIHandlers.DragMin.startPos.Y.Offset + delta.Y) 
    end 
end)

-- TOP BAR (Title + Controls)
local TopBar = Instance.new("Frame", Main); TopBar.Size=UDim2.new(1,0,0,40); TopBar.BackgroundTransparency=1
local AppTitle = Instance.new("TextLabel", TopBar); AppTitle.Size=UDim2.new(0,200,1,0); AppTitle.Position=UDim2.new(0,15,0,0); AppTitle.BackgroundTransparency=1; AppTitle.TextSize=16; AppTitle.Font=Enum.Font.GothamBold; AppTitle.TextXAlignment=Enum.TextXAlignment.Left; AppTitle.RichText=true
...
task.spawn(function()
    ...
    while AppTitle and AppTitle.Parent do
        ...
        local r, g, b = math.floor(c.R*255), math.floor(c.G*255), math.floor(c.B*255)
        AppTitle.Text = string.format('<font color="rgb(%d,%d,%d)">STARSHIP</font> <font color="rgb(180,180,190)" size="11">PREMIUM</font>', r, g, b)
        task.wait(0.03)
    end
end)
...
local function ToggleMin()
    if Main.Visible then
        ...
        -- Effect
        TweenService:Create(MinIcon, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Size=UDim2.new(0,50,0,50)}):Play()
    else
        -- Restore
        Main.Visible = true
        MinIcon.Visible = false
...
local CloseBtn = Instance.new("TextButton", TopBar); CloseBtn.Text="×"; CloseBtn.Size=UDim2.new(0,topBtnSize,1,0); CloseBtn.Position=UDim2.new(1,-topBtnSize,0,0); CloseBtn.BackgroundTransparency=1; CloseBtn.TextColor3=C_RED; CloseBtn.TextSize=22; CloseBtn.Font=Enum.Font.GothamMedium
CloseBtn.MouseButton1Click:Connect(function() 
    ShowConfirm("EXIT STARSHIP", "Are you sure you want to close the script completely?", function() 
        if getgenv().ToggleNametags then getgenv().ToggleNametags(false) end
        if ToggleMinConnection then ToggleMinConnection:Disconnect() end
        if UIHandlers.CleanupSpeedDisplay then UIHandlers.CleanupSpeedDisplay() end
        ScreenGui:Destroy()
    end) 
end)

local MiniBtn = Instance.new("TextButton", TopBar); MiniBtn.Text="—"; MiniBtn.Size=UDim2.new(0,topBtnSize,1,0); MiniBtn.Position=UDim2.new(1,-topBtnSize*2,0,0); MiniBtn.BackgroundTransparency=1; MiniBtn.TextColor3=C_TEXT_DIM; MiniBtn.TextSize=18; MiniBtn.Font=Enum.Font.GothamMedium
...
task.spawn(function()
    ...
    while TopBar and TopBar.Parent do
        ...
        TS.SessLbl.Text = string.format("⏱️ %02d:%02d:%02d", math.floor(diff/3600), math.floor((diff%3600)/60), diff%60)
...
TopBar.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
        ...
        input.Changed:Connect(function()
            if input.UserInputState == Enum.UserInputState.End then UIHandlers.Drag.dragging = false end
        end)
    end
end)
...
UserInputService.InputChanged:Connect(function(input)
    if UIHandlers.Drag.dragging and input == UIHandlers.Drag.dragInput then
        ...
        Main.Position = UDim2.new(UIHandlers.Drag.startPos.X.Scale, UIHandlers.Drag.startPos.X.Offset + delta.X, UIHandlers.Drag.startPos.Y.Scale, UIHandlers.Drag.startPos.Y.Offset + delta.Y)
    end
end)

-- SIDEBAR
local Sidebar = Instance.new("Frame", Main); Sidebar.Size=UDim2.new(0,Mobile.SIDEBAR_WIDTH,1,-40); Sidebar.Position=UDim2.new(0,0,0,40); Sidebar.BackgroundColor3=C_SIDE; Sidebar.BorderSizePixel=0
local SidebarCorner = Instance.new("UICorner", Sidebar); SidebarCorner.CornerRadius=UDim.new(0,0) -- Flat left side
...
if Mobile.SIDEBAR_COLLAPSED then
    do
        ...
        SidebarToggle.MouseButton1Click:Connect(function()
            ...
            TweenService:Create(Sidebar, TweenInfo.new(0.3, Enum.EasingStyle.Quad), {Size = UDim2.new(0, targetWidth, 1, -40)}):Play()
...
local TabContainer = Instance.new("Frame", Sidebar); TabContainer.Size=UDim2.new(1,0,1,-100); TabContainer.Position=UDim2.new(0,0,0,10); TabContainer.BackgroundTransparency=1
local UIListTabs = Instance.new("UIListLayout", TabContainer); UIListTabs.FillDirection=Enum.FillDirection.Vertical; UIListTabs.SortOrder=Enum.SortOrder.LayoutOrder; UIListTabs.Padding=UDim.new(0,8); UIListTabs.HorizontalAlignment=Enum.HorizontalAlignment.Center
...
do
    ...
    task.spawn(function()
        ...
        if content then 
            ...
            if content2 then ProfileImg.Image = content2 end
        end
    end)
    ...
    task.spawn(function()
        ...
        if not isOwner then
            ...
            if isOwner then rankText = "OWNER" end
        end
...
UIHandlers.ShowSaveMergeModal = function(callback)
    ...
    local stroke = Instance.new("UIStroke", Box); stroke.Color = C_ACCENT
...
-- RESIZE HANDLE
UIHandlers.ResizeHandle = Instance.new("TextButton", Main); UIHandlers.ResizeHandle.Text="◢"; UIHandlers.ResizeHandle.Size=UDim2.new(0,20,0,20); UIHandlers.ResizeHandle.Position=UDim2.new(1,-20,1,-20); UIHandlers.ResizeHandle.BackgroundTransparency=1; UIHandlers.ResizeHandle.TextColor3=C_TEXT_DIM; UIHandlers.ResizeHandle.TextSize=14; UIHandlers.ResizeHandle.ZIndex=100
UIHandlers.Resize = {resizing = false, dragStart = nil, startSize = nil}
MinIcon.MouseButton1Click:Connect(function()
    if not UIHandlers.DragMin.hasDragged then ToggleMin() end
end)
ToggleMinConnection = UserInputService.InputBegan:Connect(function(input) if not isBinding and not UserInputService:GetFocusedTextBox() and input.KeyCode==Config.Keybinds.ToggleMinimize then ToggleMin() end end)

local currentTab = nil
local function SwitchTab(name)
    currentTab = name
    if PageDashboard then PageDashboard.Visible=(name=="Dashboard") end
    if PageRecord then PageRecord.Visible=(name=="Recorder") end
    if PagePlay then PagePlay.Visible=(name=="List Recorder") end
    if PageMerge then 
        PageMerge.Visible=(name=="Merger") 
        if name == "Merger" and UIHandlers.RefreshMergerList then UIHandlers.RefreshMergerList() end
    end
    if PageListMap then PageListMap.Visible=(name=="List Map") end
    if PageTools then PageTools.Visible=(name=="Tools") end
    if PageWarp then PageWarp.Visible=(name=="Warp") end
    if PageHelper then PageHelper.Visible=(name=="Helper") end
    if PageFun then PageFun.Visible=(name=="Fun") end
    if PageConfig then PageConfig.Visible=(name=="Config") end
    ...
    for _,btn in pairs(TabBtns) do
        ...
        -- Animate Background
        TweenService:Create(btn, TweenInfo.new(0.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency=targetTrans}):Play()
...
local function CreateTab(name, icon, page)
    ...
    btn.Size = Mobile.SIDEBAR_COLLAPSED and UDim2.new(0, Mobile.TOUCH_MIN_SIZE, 0, btnHeight) or UDim2.new(0.85, 0, 0, btnHeight)
    ...
    btn.MouseEnter:Connect(function() 
        if currentTab ~= name then 
            if lbl.Visible then TweenService:Create(lbl, TweenInfo.new(0.2), {TextColor3 = C_TEXT}):Play() end
            if iconLbl then TweenService:Create(iconLbl, TweenInfo.new(0.2), {TextColor3 = C_TEXT}):Play() end
            TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundTransparency = 0.9}):Play()
        end 
    end)
    btn.MouseLeave:Connect(function() 
        if currentTab ~= name then
            if lbl.Visible then TweenService:Create(lbl, TweenInfo.new(0.2), {TextColor3 = C_TEXT_DIM}):Play() end
            if iconLbl then TweenService:Create(iconLbl, TweenInfo.new(0.2), {TextColor3 = C_TEXT_DIM}):Play() end
...
PageRecord = Instance.new("Frame", ContentArea); PageRecord.Size=UDim2.new(1,0,1,0); PageRecord.Position=UDim2.new(0,0,0,0); PageRecord.BackgroundTransparency=1; PageRecord.Visible=true
PageMerge = Instance.new("Frame", ContentArea); PageMerge.Size=UDim2.new(1,0,1,0); PageMerge.Position=UDim2.new(0,0,0,0); PageMerge.BackgroundTransparency=1; PageMerge.Visible=false
PageListMap = Instance.new("Frame", ContentArea); PageListMap.Size=UDim2.new(1,0,1,0); PageListMap.Position=UDim2.new(0,0,0,0); PageListMap.BackgroundTransparency=1; PageListMap.Visible=false
PageTools = Instance.new("Frame", ContentArea); PageTools.Size=UDim2.new(1,0,1,0); PageTools.Position=UDim2.new(0,0,0,0); PageTools.BackgroundTransparency=1; PageTools.Visible=false
PageWarp = Instance.new("Frame", ContentArea); PageWarp.Size=UDim2.new(1,0,1,0); PageWarp.Position=UDim2.new(0,0,0,0); PageWarp.BackgroundTransparency=1; PageWarp.Visible=false
PageHelper = Instance.new("Frame", ContentArea); PageHelper.Size=UDim2.new(1,0,1,0); PageHelper.Position=UDim2.new(0,0,0,0); PageHelper.BackgroundTransparency=1; PageHelper.Visible=false
PageFun = Instance.new("Frame", ContentArea); PageFun.Size=UDim2.new(1,0,1,0); PageFun.Position=UDim2.new(0,0,0,0); PageFun.BackgroundTransparency=1; PageFun.Visible=false
PageConfig = Instance.new("Frame", ContentArea); PageConfig.Size=UDim2.new(1,0,1,0); PageConfig.Position=UDim2.new(0,0,0,0); PageConfig.BackgroundTransparency=1; PageConfig.Visible=false
PageDashboard = Instance.new("Frame", ContentArea); PageDashboard.Size=UDim2.new(1,0,1,0); PageDashboard.Position=UDim2.new(0,0,0,0); PageDashboard.BackgroundTransparency=1; PageDashboard.Visible=false

-- === INIT TAB MODULES ===
-- Load and execute the modular tabs here
local WarpTab = LoadModule("Tabs/Warp")
if WarpTab then WarpTab(PageWarp, UIModule, Connections, Config, LocalPlayer, UIHandlers, ShowConfirm, RegisterTheme) end

local FunTab = LoadModule("Tabs/Fun")
if FunTab then FunTab(PageFun, UIModule, Connections, Config, LocalPlayer, UIHandlers, RegisterTheme) end

local ToolsTab = LoadModule("Tabs/Tools")
if ToolsTab then ToolsTab(PageTools, UIModule, Connections, Config, LocalPlayer, UIHandlers, RegisterTheme) end

local HelperTab = LoadModule("Tabs/Helper")
if HelperTab then HelperTab(PageHelper, UIModule, Connections, Config, LocalPlayer, UIHandlers, ShowConfirm, RegisterTheme) end
...
if ConfigTabModule then 
    ...
    if type(ConfigTabModule) == "table" and ConfigTabModule.SetupUI then
        ConfigTabModule.SetupUI(PageConfig, UIModule, Connections, Config, LocalPlayer, UIHandlers, Themes, ThemeObjects, Main)
        -- Force Apply Default Theme immediately to catch any missed elements
        if ConfigTabModule.ApplyTheme then ConfigTabModule.ApplyTheme(Config.Theme or "Default") end
    end
end

local DashboardTab = LoadModule("Tabs/Dashboard")
if DashboardTab then DashboardTab(PageDashboard, UIModule, Connections, Config, LocalPlayer, UIHandlers, RegisterTheme) end
...
SwitchTab("Dashboard")
local CountGui = Instance.new("ScreenGui", CoreGui); CountGui.Name="WalkGemCount"; CountGui.Enabled=false
local CountLbl = Instance.new("TextLabel", CountGui); CountLbl.Size=UDim2.new(1,0,1,0); CountLbl.BackgroundTransparency=1; CountLbl.TextColor3=C_ACCENT; CountLbl.Font=Enum.Font.GothamBold; CountLbl.TextSize=100; CountLbl.TextStrokeTransparency=0.5
...
local UIListRecord = Instance.new("UIListLayout", PageRecord); UIListRecord.SortOrder=Enum.SortOrder.LayoutOrder; UIListRecord.Padding=UDim.new(0,8)
...
do
    ...
    BtnStart.MouseButton1Click:Connect(function() UIHandlers.StartCountdown(StartRecording) end)
    BtnPause.MouseButton1Click:Connect(function() if UIHandlers.OnPauseClick then UIHandlers.OnPauseClick() end end)
    BtnStop.MouseButton1Click:Connect(function() if UIHandlers.OnStopClick then UIHandlers.OnStopClick() end end)
end
...
do
    ...
    BtnTogglePath.MouseButton1Click:Connect(function() if UIHandlers.OnTogglePathClick then UIHandlers.OnTogglePathClick() end end)
end

-- 4. Rewind UI (Hidden by default) - Wrapped in do...end to save registers
local RewindFrame = Instance.new("Frame", PageRecord); RewindFrame.Size=UDim2.new(1,0,0,70); RewindFrame.BackgroundColor3=C_ITEM; RewindFrame.Visible=false; RewindFrame.LayoutOrder=4; Instance.new("UICorner", RewindFrame).CornerRadius=UDim.new(0,8)
RegisterTheme(RewindFrame, "BackgroundColor3", "Item")

-- Rewind UI elements (outside do block for accessibility)
local RewindTimeLbl = Instance.new("TextLabel", RewindFrame); RewindTimeLbl.Text="0.0s"; RewindTimeLbl.Size=UDim2.new(1,0,0,20); RewindTimeLbl.Position=UDim2.new(0,-10,0,5); RewindTimeLbl.BackgroundTransparency=1; RewindTimeLbl.TextColor3=C_ACCENT; RewindTimeLbl.Font=Enum.Font.GothamBold; RewindTimeLbl.TextSize=12; RewindTimeLbl.TextXAlignment=Enum.TextXAlignment.Right
RegisterTheme(RewindTimeLbl, "TextColor3", "Accent")

local RewindSliderBg = Instance.new("TextButton", RewindFrame); RewindSliderBg.Text=""; RewindSliderBg.Size=UDim2.new(0.9,0,0,6); RewindSliderBg.Position=UDim2.new(0.05,0,0.45,0); RewindSliderBg.BackgroundColor3=Color3.fromRGB(30,30,35); RewindSliderBg.AutoButtonColor=false; Instance.new("UICorner", RewindSliderBg)

local RewindSliderFill = Instance.new("Frame", RewindSliderBg); RewindSliderFill.Size=UDim2.new(0.5,0,1,0); RewindSliderFill.BackgroundColor3=C_ACCENT; Instance.new("UICorner", RewindSliderFill)
...
do
    local hdr = Instance.new("TextLabel", RewindFrame); hdr.Text="REWIND CONTROL"; hdr.Size=UDim2.new(1,0,0,20); hdr.BackgroundTransparency=1; hdr.TextColor3=C_TEXT_DIM; hdr.Font=Enum.Font.GothamBold; hdr.TextSize=10; hdr.Position=UDim2.new(0,0,0,5)
    RegisterTheme(hdr, "TextColor3", "TextDim")
    
    local dragging=false
    RewindSliderBg.MouseButton1Down:Connect(function() dragging=true end)
    ...
    game:GetService("UserInputService").InputChanged:Connect(function(i) if dragging and i.UserInputType==Enum.UserInputType.MouseMovement and (isRecording and isPaused) then RewindUpdateSlider(i) end end)
    
    local btnResume = Instance.new("TextButton", RewindFrame); btnResume.Text="RESUME RECORDING"; btnResume.Size=UDim2.new(0.9,0,0,25); btnResume.Position=UDim2.new(0.05,0,0.65,0); btnResume.BackgroundColor3=C_GREEN; btnResume.TextColor3=Color3.new(1,1,1); Instance.new("UICorner", btnResume)
...
(function()
    ...
    local FileTitle = Instance.new("TextLabel", FileHeader); FileTitle.Text = "  RECORDINGS"; FileTitle.Size = UDim2.new(0.3, 0, 1, 0); FileTitle.BackgroundTransparency = 1; FileTitle.TextColor3 = C_TEXT_DIM; FileTitle.Font = Enum.Font.GothamBold; FileTitle.TextSize = 10; FileTitle.TextXAlignment = Enum.TextXAlignment.Left
    ...
    local PRefresh = Instance.new("TextButton", FileHeader); PRefresh.Text = "REFRESH"; PRefresh.Size = UDim2.new(0, 60, 0, 20); PRefresh.Position = UDim2.new(1, -65, 0, 7); PRefresh.BackgroundColor3 = C_MAIN; PRefresh.TextColor3 = C_TEXT; PRefresh.Font = Enum.Font.GothamBold; PRefresh.TextSize = 9; Instance.new("UICorner", PRefresh).CornerRadius = UDim.new(0, 4)
    RegisterTheme(PRefresh, "BackgroundColor3", "Main")
    RegisterTheme(PRefresh, "TextColor3", "Text")

    local PScroll = Instance.new("ScrollingFrame", FileListCard); PScroll.Size = UDim2.new(1, -10, 1, -35); PScroll.Position = UDim2.new(0, 5, 0, 35); PScroll.BackgroundTransparency = 1; PScroll.BorderSizePixel = 0; PScroll.ScrollBarThickness = 4; Instance.new("UIListLayout", PScroll).Padding = UDim.new(0, 4)
    ...
    RefreshPlayerList = function()
        for _, c in pairs(PScroll:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end
        local path = RECORDER_FOLDER .. "/" .. currentWorkspace
                ...
                if string.match(f, "%.json$") then
                    ...
                    btnDel.MouseButton1Click:Connect(function()
                        ShowConfirm("DELETE FILE", "Delete " .. n .. "?", function()
                            ...
                            if UIHandlers.RefreshMergerList then UIHandlers.RefreshMergerList() end
    ...
    local function UpdatePWSList()
        for _, c in pairs(PWSScroll:GetChildren()) do if c:IsA("TextButton") or c:IsA("TextBox") then c:Destroy() end end
    ...
    PInpWorkspace.MouseButton1Click:Connect(function()
        ...
        if isPWSOpen then UpdatePWSList() end
    end)
    ...
    ShowSaveRecordingModal = function()
        ...
        BtnWS.MouseButton1Click:Connect(function()
            ...
            if isWSListOpen then
                for _, c in pairs(WSList:GetChildren()) do if c:IsA("TextButton") or c:IsA("TextBox") then c:Destroy() end end
                ...
                NewWSInput.FocusLost:Connect(function(enter)
                    if enter and NewWSInput.Text ~= "" then
                        ...
                        if BtnWorkspaceDropdown then BtnWorkspaceDropdown.Text = currentWorkspace end
                        if PInpWorkspace then PInpWorkspace.Text = currentWorkspace end
                        RefreshPlayerList()
                    end
                end)
...
UIHandlers.OnPauseClick = function()
    if isRecording and not isPaused then 
        isPaused=true; RewindFrame.Visible=true
        local c=LocalPlayer.Character
        if c then 
            local r=c:FindFirstChild("HumanoidRootPart"); if r then r.Anchored=true end
        ...
        Connections.Preview = game:GetService("RunService").RenderStepped:Connect(function() 
            local c=LocalPlayer.Character; local r=c and c:FindFirstChild("HumanoidRootPart"); if not r then return end
            if #recordedData.Frames == 0 then return end
            
            -- Find frame pair for interpolation
            local fA, fB
            for i=1,#recordedData.Frames-1 do 
                if recordedData.Frames[i].t<=RewindSelectedTime and recordedData.Frames[i+1].t>=RewindSelectedTime then 
                    fA=recordedData.Frames[i]; fB=recordedData.Frames[i+1]; break 
                end 
            end
            ...
            if fA and fB then 
                ...
                if fA.r and fB.r then
                    ...
                    local j=GetJoints(c); for _,m in ipairs(j) do if fA.j and fA.j[m.Name] and fB.j and fB.j[m.Name] then local t=TblToCF(fA.j[m.Name]):Lerp(TblToCF(fB.j[m.Name]),a); m.Transform = isStrictRetarget and t.Rotation or t end end
                elseif fA.pos and fB.pos then
                    -- Flexible mode: Position data
                    local pos = Vector3.new(fA.pos.x, fA.pos.y, fA.pos.z):Lerp(Vector3.new(fB.pos.x, fB.pos.y, fB.pos.z), a)
            ...
            elseif fA then 
                ...
                if fA.r then
                    ...
                    local j=GetJoints(c); for _,m in ipairs(j) do if fA.j and fA.j[m.Name] then local t=TblToCF(fA.j[m.Name]); m.Transform = isStrictRetarget and t.Rotation or t end end
                elseif fA.pos then
    ...
    if isPathEnabled then
        ...
        elseif isRecording then
             ...
             if recordedData.Frames and #recordedData.Frames > 1 then
                 ...
                 if lastPos then lastPathPoint = lastPos end
             end
        end
    else
        ClearPath()
...
ScreenGui.AncestryChanged:Connect(function()
    if not ScreenGui.Parent then CleanupConnections() end
end)
...
UIHandlers.UpdateMWSList = function()
    if not MergerRefs.MWSScroll then return end
    for _, c in pairs(MergerRefs.MWSScroll:GetChildren()) do 
        if c:IsA("TextButton") or c:IsA("TextBox") then c:Destroy() end 
    end
    ...
    NewWSInput.FocusLost:Connect(function(enter)
        if enter and NewWSInput.Text ~= "" then
            ...
            if MergerRefs.MInpWorkspace then MergerRefs.MInpWorkspace.Text = MergerRefs.currentMergerWorkspace end
            if MergerRefs.MWSList then MergerRefs.MWSList.Visible = false end
            if UIHandlers.RefreshMergerList then UIHandlers.RefreshMergerList() end
        end
    end)
...
UIHandlers.RefreshMergerList = function()
    if not MergerRefs.MScroll then return end
    ...
    if isfolder(wsPath) then
        ...
        for _, m in ipairs(MergerRefs.mergeList) do
            ...
            for _, f in ipairs(jsonFiles) do
                if f == m.p then exists = true break end
            end
            if exists then table.insert(validMergeList, m) end
        end
        ...
        for idx, f in ipairs(jsonFiles) do
            ...
            if not isSelected then RegisterTheme(row, "BackgroundColor3", "Item") end
            ...
            if not isSelected then RegisterTheme(checkbox, "BackgroundColor3", "Main") end
            if not isSelected then RegisterTheme(checkStroke, "Color", "Accent") end
            ...
            if not isSelected then RegisterTheme(nameLabel, "TextColor3", "Text") end
...
function UIHandlers.InitMergerUI()
    ...
    local UIListMerge = Instance.new("UIListLayout", PageMerge); UIListMerge.SortOrder=Enum.SortOrder.LayoutOrder; UIListMerge.Padding=UDim.new(0,8)
    ...
    local MergeTitle = Instance.new("TextLabel", MergeHeader); MergeTitle.Text = "  SELECT FILES TO MERGE"; MergeTitle.Size = UDim2.new(0.5, 0, 1, 0); MergeTitle.BackgroundTransparency = 1; MergeTitle.TextColor3 = C_TEXT_DIM; MergeTitle.Font = Enum.Font.GothamBold; MergeTitle.TextSize = 10; MergeTitle.TextXAlignment = Enum.TextXAlignment.Left
    RegisterTheme(MergeTitle, "TextColor3", "TextDim")

    local MRefresh = Instance.new("TextButton", MergeHeader); MRefresh.Text = "↻"; MRefresh.Size = UDim2.new(0, 25, 0, 20); MRefresh.Position = UDim2.new(1, -30, 0, 5); MRefresh.BackgroundColor3 = C_MAIN; MRefresh.TextColor3 = C_TEXT; MRefresh.Font = Enum.Font.GothamBold; MRefresh.TextSize = 14; Instance.new("UICorner", MRefresh).CornerRadius = UDim.new(0, 4)
...
local _ignored_UpdateMWSList = function()
    for _, c in pairs(MergerRefs.MWSScroll:GetChildren()) do if c:IsA("TextButton") or c:IsA("TextBox") then c:Destroy() end end
...
UIHandlers.UpdateMWSList = _ignored_UpdateMWSList
    ...
    if MergerRefs.MWSList.Visible then UIHandlers.UpdateMWSList() end
end)
    ...
    MergerRefs.MScroll = Instance.new("ScrollingFrame", MergeCard); MergerRefs.MScroll.Size = UDim2.new(1, -10, 1, -35); MergerRefs.MScroll.Position = UDim2.new(0, 5, 0, 30); MergerRefs.MScroll.BackgroundTransparency = 1; MergerRefs.MScroll.BorderSizePixel = 0; MergerRefs.MScroll.ScrollBarThickness = 4; Instance.new("UIListLayout", MergerRefs.MScroll).Padding = UDim.new(0, 4)
    ...
    MMergeBtn = Instance.new("TextButton", ActionCard); MMergeBtn.Text = "MERGE SELECTED FILES"; MMergeBtn.Size = UDim2.new(0.9, 0, 0, 40); MMergeBtn.Position = UDim2.new(0.05, 0, 0, 10); MMergeBtn.BackgroundColor3 = C_ACCENT; MMergeBtn.TextColor3 = Color3.new(1,1,1); MMergeBtn.Font = Enum.Font.GothamBold; Instance.new("UICorner", MMergeBtn).CornerRadius = UDim.new(0, 6)
    ...
    MMergeBtn.MouseButton1Click:Connect(function()
        ...
        UIHandlers.ShowSaveMergeModal(function(name)
            task.spawn(function()
                        ...
                        if frames and #frames > 0 then
                            ...
                            if #finalFrames > 0 then
                                ...
                                for k = #finalFrames, 1, -1 do
                                    if k % 100 == 0 then task.wait() end -- More frequent yielding
                                    
                                    local f = finalFrames[k]
                                    local isMoving = false
                                    if f.vel and (math.abs(f.vel.x) > 1.0 or math.abs(f.vel.z) > 1.0) then isMoving = true end
                                    
                                    if not isMoving and k > 1 then
                                        local prev = finalFrames[k-1]
                                        local p1 = (f.pos and Vector3.new(f.pos.x, 0, f.pos.z)) or (f.r and Vector3.new(f.r[1], 0, f.r[3]))
                                        local p2 = (prev.pos and Vector3.new(prev.pos.x, 0, prev.pos.z)) or (prev.r and Vector3.new(prev.r[1], 0, prev.r[3]))
                                        if p1 and p2 and (p1 - p2).Magnitude > 0.05 then isMoving = true end
                                    end
                                    
                                    if isMoving then lastKeepIndex = k; break end
                                end
                            ...
                            local trimTime = 0
                            if true then 
                                local startPos = (frames[1].pos and Vector3.new(frames[1].pos.x, frames[1].pos.y, frames[1].pos.z)) or (frames[1].r and TblToCF(frames[1].r).Position)
                                for j, f in ipairs(frames) do
                                    if j % 100 == 0 then task.wait() end -- More frequent yielding
                                    
                                    local currPos = (f.pos and Vector3.new(f.pos.x, f.pos.y, f.pos.z)) or (f.r and TblToCF(f.r).Position)
                                    if startPos and currPos and (currPos - startPos).Magnitude > 0.5 then
                                        trimTime = f.t
                                        break
                                    end
                                end
                                if trimTime >= frames[#frames].t then trimTime = 0 end
                            end
                            ...
                            for j, f in ipairs(frames) do
                                ...
                                if f.t >= trimTime then
                                    local newFrame = {
                                        ...
                                        r = f.r, j = f.j, pos = f.pos, rot = f.rot, vel = f.vel,
                                        md = f.md, st = f.st, jmp = f.jmp, hh = f.hh,
                                        -- Shiftlock / Camera data
                                        camLook = f.camLook, shiftlock = f.shiftlock, charLook = f.charLook
                                    }
                ...
                if SwitchTab then SwitchTab("List Map") end
            end)
        end)
    end)

    MRefresh.MouseButton1Click:Connect(function() MergerRefs.mergeList={}; UIHandlers.RefreshMergerList() end)
    UIHandlers.RefreshMergerList()
end
...
function UIHandlers.SetupListMapUI()
    ...
    UserInputService.InputChanged:Connect(function(input) 
        if input == dragInput and dragging then 
            ...
            MapPlayerPopup.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y) 
        end 
    end)
    ...
    do
        ...
        BtnZoom.MouseButton1Click:Connect(function()
            ...
            if isZoomPunch then ShowToast("Zoom Punch", "FOV punch on jumps/landings", "info", 2) end
        end)
        ...
        BtnRespawn.MouseButton1Click:Connect(function()
            ...
            if isRespawnOnEnd then ShowToast("Respawn On End", "Character will respawn when playback ends", "info", 2) end
        end)
    end
    ...
    BtnSpinbot.MouseButton1Click:Connect(function()
        ...
        if isFreestyle then
            freestyleLoop = RunService.RenderStepped:Connect(function(dt)
                if not isPlaying then return end -- Only spin when playing
            ...
            if freestyleLoop then freestyleLoop:Disconnect(); freestyleLoop = nil end
    ...
    local function CreateFileCard(fileName, index)
        ...
        if currentPlaybackFile ~= fileName then RegisterTheme(FileCard, "TextColor3", "Text") end
        ...
        BtnDelete.MouseButton1Click:Connect(function()
            if ShowConfirm then
                ShowConfirm("DELETE FILE", "Are you sure you want to delete: " .. fileName:gsub(".json", "") .. "?", function()
        ...
        FileCard.MouseButton1Click:Connect(function()
            if currentPlaybackFile == fileName then
                ...
                task.spawn(function()
                    ...
                    local success, data = pcall(function() return HttpService:JSONDecode(readfile(MERGER_FOLDER .. "/" .. fileName)) end)
    ...
    local function RefreshMapList()
        ...
        local files = listfiles(MERGER_FOLDER)
            ...
            for index, filePath in ipairs(files) do
                ...
                if not string.match(fileName, "%.json$") then continue end
                if not isfile(filePath) then continue end
                ...
                if mapCount % 5 == 0 then task.wait() end -- Yield every 5 items to prevent lag
            end
    ...
    PBtnStop.MouseButton1Click:Connect(function()
        if currentPlaybackSource == "Merger" then
            ...
            isPlaying = false
            isPlayPaused = false
            if Connections.Playback then Connections.Playback:Disconnect() end
            currentPlaybackTime = 0
            
            -- Reset Character and Path
            if ResetChar then ResetChar() end
            if ClearPath then ClearPath() end
    ...
    PSliderBg.MouseButton1Down:Connect(function() draggingP=true end)
    game:GetService("UserInputService").InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then draggingP=false end end)
    game:GetService("UserInputService").InputChanged:Connect(function(i) 
        if draggingP and i.UserInputType==Enum.UserInputType.MouseMovement and (isPlaying or isPlayPaused) and currentPlaybackSource == "Merger" then 
    ...
    game:GetService("RunService").RenderStepped:Connect(function()
        if not PageListMap.Visible then return end
...
function UIHandlers.SetupNametags()
    ...
    local function AddTag(char, data, player)
        ...
        if head:FindFirstChild("StarshipTag") then head.StarshipTag:Destroy() end
        ...
        task.spawn(function()
            while bg and bg.Parent do
                if math.random() > 0.6 then -- Spawn rate
                    ...
                    local t = TweenService:Create(flake, TweenInfo.new(duration, Enum.EasingStyle.Linear), {Position = targetPos})
                    t:Play()
        ...
        strokeGrad.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.new(1,1,1)), ColorSequenceKeypoint.new(1, Color3.new(1,1,1))}
    ...
    local function ApplyTagForPlayer(player)
        ...
        if not ActiveScriptUsers[player.UserId] then return end
        if not SpecialUsers[player.UserId] then return end
    ...
    local function StartHeartbeat()
        if heartbeatLoop then return end
    ...
    LocalPlayer.CharacterAdded:Connect(function() task.wait(1); ApplySelfTag() end)
...
UIHandlers.ToggleIndicator = function()
    ...
    if now - UIHandlers.CinemaMode.lastF9Press < 0.5 then
        ...
        if ScreenGui:FindFirstChild("HiddenIndicator") then
            ScreenGui.HiddenIndicator.Visible = UIHandlers.CinemaMode.showIndicator and UIHandlers.CinemaMode.isMainUIHidden
        end
...
Connections.MainKeybind = UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if not gameProcessed and not isBinding then
        if input.KeyCode == Enum.KeyCode.F2 then
            if not UIHandlers.ToggleIndicator() then UIHandlers.ToggleMainUI() end
            return
        end
        ...
        if Config.Keybinds.StartRecording and input.KeyCode == Config.Keybinds.StartRecording then
            if UIHandlers.ToggleRecording then UIHandlers.ToggleRecording() end
        end
        if Config.Keybinds.PauseRecording and input.KeyCode == Config.Keybinds.PauseRecording then
            if UIHandlers.OnPauseClick then UIHandlers.OnPauseClick() end
        end
        if Config.Keybinds.TogglePath and input.KeyCode == Config.Keybinds.TogglePath then
            if UIHandlers.OnTogglePathClick then UIHandlers.OnTogglePathClick() end
        end
        if Config.Keybinds.PlayPlayback and input.KeyCode == Config.Keybinds.PlayPlayback then
            if PageListMap and PageListMap.Visible and currentPlaybackFile then
                if isPlaying and currentPlaybackSource == "Merger" then 
                    if PausePlayback then PausePlayback() end
                elseif currentPlaybackFile then 
                    if isfile(MERGER_FOLDER .. "/" .. currentPlaybackFile) then
                        if UIHandlers.PlayMergerRecording then UIHandlers.PlayMergerRecording(currentPlaybackFile, not isPlayPaused) end
                    end
                end 
            end
        end
...
local DEV_MODE = false -- Set to false for production
