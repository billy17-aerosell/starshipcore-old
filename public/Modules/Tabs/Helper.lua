local function SetupHelperUI(PageHelper, UI, Connections, Config, LocalPlayer, UIHandlers, ShowConfirm, RegisterTheme)
    ...
    -- 7. STRAFE ASSIST
    local CardStrafe = CreateCard("STRAFE ASSIST", 135, 7)
    local BtnStrafe = Instance.new("TextButton", CardStrafe)
    BtnStrafe.Text = "STRAFE ASSIST: OFF"
    BtnStrafe.Size = UDim2.new(0.94, 0, 0, 35)
    BtnStrafe.Position = UDim2.new(0.03, 0, 0, 35)
    StyleBtn(BtnStrafe, C_RED)
    local LblStrafePower = Instance.new("TextLabel", CardStrafe)
    LblStrafePower.Text = "POWER: 1.5x"
    LblStrafePower.Size = UDim2.new(1, -20, 0, 20)
    LblStrafePower.Position = UDim2.new(0, 15, 0, 80)
    LblStrafePower.BackgroundTransparency = 1
    LblStrafePower.TextColor3 = C_TEXT_DIM
    LblStrafePower.Font = Enum.Font.GothamBold
    LblStrafePower.TextSize = 10
    LblStrafePower.TextXAlignment = Enum.TextXAlignment.Left
    local SldStrafeBg = Instance.new("TextButton", CardStrafe)
    SldStrafeBg.Text = ""
    SldStrafeBg.Size = UDim2.new(0.9, 0, 0, 6)
    SldStrafeBg.Position = UDim2.new(0.05, 0, 0, 100)
    SldStrafeBg.BackgroundColor3 = C_SIDE
    SldStrafeBg.AutoButtonColor = false
    Instance.new("UICorner", SldStrafeBg).CornerRadius = UDim.new(0, 3)
    local SldStrafeFill = Instance.new("Frame", SldStrafeBg)
    SldStrafeFill.Size = UDim2.new(0.5, 0, 1, 0)
    SldStrafeFill.BackgroundColor3 = C_ACCENT
    Instance.new("UICorner", SldStrafeFill).CornerRadius = UDim.new(0, 3)
    local strafePower, isStrafing, strafeLoop = 1.5, false, nil
    local function UpdateStrafeSlider(input)
        local rx = input.Position.X - SldStrafeBg.AbsolutePosition.X
        local sc = math.clamp(rx / Sld StrafeBg.AbsoluteSize.X, 0, 1)
        strafePower = 1.0 + (sc * 2.0)
        SldStrafeFill.Size = UDim2.new(sc, 0, 1, 0)
        LblStrafePower.Text = string.format("POWER: %.1fx", strafePower)
    end
    local draggingStrafe = false
    SldStrafeBg.MouseButton1Down:Connect(function() draggingStrafe = true end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then draggingStrafe = false end end)
    UserInputService.InputChanged:Connect(function(i) if draggingStrafe and i.UserInputType == Enum.UserInputType.MouseMovement then UpdateStrafeSlider(i) end end)
    BtnStrafe.MouseButton1Click:Connect(function()
        isStrafing = not isStrafing
        BtnStrafe.Text = "STRAFE ASSIST: " .. (isStrafing and "ON" or "OFF")
        BtnStrafe.TextColor3 = isStrafing and C_GREEN or C_RED
        BtnStrafe.UIStroke.Color = isStrafing and C_GREEN or C_RED
        if isStrafing then
            strafeLoop = RunService.Heartbeat:Connect(function()
                local c, h, r = LocalPlayer.Character, nil, nil
                if c then h = c:FindFirstChild("Humanoid"); r = c:FindFirstChild("HumanoidRootPart") end
                if h and r and (h:GetState() == Enum.HumanoidStateType.Jumping or h:GetState() == Enum.HumanoidStateType.Freefall) and h.MoveDirection.Magnitude > 0.1 then
                    local vel = r.AssemblyLinearVelocity
                    ...
                    if math.abs(inputX) > 0.1 then
                        local strafeDir = (camCF.LookVector * inputZ + camCF.RightVector * inputX).Unit
                        local newVel = horizVel:Lerp(strafeDir * speed * strafePower, 0.1)
                        r.AssemblyLinearVelocity = Vector3.new(newVel.X, vel.Y, newVel.Z)
                    end
                end
            end)
            table.insert(Connections, strafeLoop)
        else
            if strafeLoop then strafeLoop:Disconnect(); strafeLoop = nil end
        end
    end)

    -- 8. DASH ABILITY  
    local CardDash = CreateCard("DASH ABILITY", 165, 8)
    local BtnDash = Instance.new("TextButton", CardDash)
    BtnDash.Text = "DASH: OFF"
    BtnDash.Size = UDim2.new(0.94, 0, 0, 35)
    BtnDash.Position = UDim2.new(0.03, 0, 0, 35)
    StyleBtn(BtnDash, C_RED)
    local LblDashPower = Instance.new("TextLabel", CardDash)
    LblDashPower.Text = "POWER: 50"
    LblDashPower.Size = UDim2.new(0.48, 0, 0, 20)
    LblDashPower.Position = UDim2.new(0, 15, 0, 80)
    LblDashPower.BackgroundTransparency = 1
    LblDashPower.TextColor3 = C_TEXT_DIM
    LblDashPower.Font = Enum.Font.GothamBold
    LblDashPower.TextSize = 10
    LblDashPower.TextXAlignment = Enum.TextXAlignment.Left
    local LblDashCooldown = Instance.new("TextLabel", CardDash)
    LblDashCooldown.Text = "COOLDOWN: 0.5s"
    LblDashCooldown.Size = UDim2.new(0.48, 0, 0, 20)
    LblDashCooldown.Position = UDim2.new(0.52, 0, 0, 80)
    LblDashCooldown.BackgroundTransparency = 1
    LblDashCooldown.TextColor3 = C_TEXT_DIM
    LblDashCooldown.Font = Enum.Font.GothamBold
    LblDashCooldown.TextSize = 10
    LblDashCooldown.TextXAlignment = Enum.TextXAlignment.Left
    local SldDashPowerBg = Instance.new("TextButton", CardDash)
    SldDashPowerBg.Text = ""
    SldDashPowerBg.Size = UDim2.new(0.43, 0, 0, 6)
    SldDashPowerBg.Position = UDim2.new(0.05, 0, 0, 100)
    SldDashPowerBg.BackgroundColor3 = C_SIDE
    SldDashPowerBg.AutoButtonColor = false
    Instance.new("UICorner", SldDashPowerBg).CornerRadius = UDim.new(0, 3)
    local SldDashPowerFill = Instance.new("Frame", SldDashPowerBg)
    SldDashPowerFill.Size = UDim2.new(0.5, 0, 1, 0)
    SldDashPowerFill.BackgroundColor3 = C_ACCENT
    Instance.new("UICorner", SldDashPowerFill).CornerRadius = UDim.new(0, 3)
    local SldDashCDBg = Instance.new("TextButton", CardDash)
    SldDashCDBg.Text = ""
    SldDashCDBg.Size = UDim2.new(0.43, 0, 0, 6)
    SldDashCDBg.Position = UDim2.new(0.52, 0, 0, 100)
    SldDashCDBg.BackgroundColor3 = C_SIDE
    SldDashCDBg.AutoButtonColor = false
    Instance.new("UICorner", SldDashCDBg).CornerRadius = UDim.new(0, 3)
    local SldDashCDFill = Instance.new("Frame", SldDashCDBg)
    SldDashCDFill.Size = UDim2.new(0.5, 0, 1, 0)
    SldDashCDFill.BackgroundColor3 = C_ACCENT
    Instance.new("UICorner", SldDashCDFill).CornerRadius = UDim.new(0, 3)
    local LblDashKeybind = Instance.new("TextLabel", CardDash)
    LblDashKeybind.Text = "KEYBIND: Q"
    LblDashKeybind.Size = UDim2.new(0.94, 0, 0, 18)
    LblDashKeybind.Position = UDim2.new(0.03, 0, 0, 115)
    LblDashKeybind.BackgroundTransparency = 1
    LblDashKeybind.TextColor3 = C_TEXT_DIM
    LblDashKeybind.Font = Enum.Font.Code
    LblDashKeybind.TextSize = 10
    LblDashKeybind.TextXAlignment = Enum.TextXAlignment.Left
    local BtnSetDashKey = Instance.new("TextButton", CardDash)
    BtnSetDashKey.Text = "SET KEY"
    BtnSetDashKey.Size = UDim2.new(0.94, 0, 0, 25)
    BtnSetDashKey.Position = UDim2.new(0.03, 0, 0, 135)
    StyleBtn(BtnSetDashKey, C_ACCENT)
    local dashPower, dashCooldown, dashKey, isDashEnabled, lastDash, isSettingDashKey = 50, 0.5, Enum.KeyCode.Q, false, 0, false
    local dashInputConn = nil
    local function UpdateDashPowerSlider(input)
        local sc = math.clamp((input.Position.X - SldDashPowerBg.AbsolutePosition.X) / SldDashPowerBg.AbsoluteSize.X, 0, 1)
        dashPower = 20 + (sc * 80)
        SldDashPowerFill.Size = UDim2.new(sc, 0, 1, 0)
        LblDashPower.Text = string.format("POWER: %d", math.floor(dashPower))
    end
    local function UpdateDashCDSlider(input)
        local sc = math.clamp((input.Position.X - SldDashCDBg.AbsolutePosition.X) / SldDashCDBg.AbsoluteSize.X, 0, 1)
        dashCooldown = 0.1 + (sc * 1.9)
        SldDashCDFill.Size = UDim2.new(sc, 0, 1, 0)
        LblDashCooldown.Text = string.format("COOLDOWN: %.1fs", dashCooldown)
    end
    local draggingDashPower, draggingDashCD = false, false
    SldDashPowerBg.MouseButton1Down:Connect(function() draggingDashPower = true end)
    SldDashCDBg.MouseButton1Down:Connect(function() draggingDashCD = true end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then draggingDashPower, draggingDashCD = false, false end end)
    UserInputService.InputChanged:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseMovement then if draggingDashPower then UpdateDashPowerSlider(i) end if draggingDashCD then UpdateDashCDSlider(i) end end end)
    BtnSetDashKey.MouseButton1Click:Connect(function()
        if isSettingDashKey then return end
        isSettingDashKey = true
        BtnSetDashKey.Text = "PRESS ANY KEY..."
        BtnSetDashKey.TextColor3 = C_YELLOW
        BtnSetDashKey.UIStroke.Color = C_YELLOW
        local conn
        conn = UserInputService.InputBegan:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Keyboard then
                dashKey = input.KeyCode
                LblDashKeybind.Text = "KEYBIND: " .. dashKey.Name
                BtnSetDashKey.Text = "SET KEY"
                BtnSetDashKey.TextColor3 = C_ACCENT
                BtnSetDashKey.UIStroke.Color = C_ACCENT
                isSettingDashKey = false
                conn:Disconnect()
            end
        end)
    end)
    local function PerformDash()
        local c, r = LocalPlayer.Character, nil
        if c then r = c:FindFirstChild("HumanoidRootPart") end
        if not r or tick() - lastDash < dashCooldown then return end
        lastDash = tick()
        local camCF = workspace.CurrentCamera.CFrame
        local inputX, inputZ = 0, 0
        ...
        local dashDir = camCF.LookVector
        if inputX ~= 0 or inputZ ~= 0 then dashDir = (camCF.LookVector * inputZ + camCF.RightVector * inputX).Unit end
        local currentVel = r.AssemblyLinearVelocity
        r.AssemblyLinearVelocity = Vector3.new(dashDir.X * dashPower, currentVel.Y, dashDir.Z * dashPower)
        local effect = Instance.new("Part")
        effect.Size = Vector3.new(2, 2, 2)
        effect.Position = r.Position
        effect.Anchored = true
        effect.CanCollide = false
        effect.Transparency = 0.5
        effect.Color = C_ACCENT
        effect.Material = Enum.Material.Neon
        effect.Parent = workspace
        TweenService:Create(effect, TweenInfo.new(0.3), {Transparency = 1, Size = Vector3.new(4, 4, 4)}):Play()
        task.delay(0.3, function() effect:Destroy() end)
    end
    BtnDash.MouseButton1Click:Connect(function()
        isDashEnabled = not isDashEnabled
        BtnDash.Text = "DASH: " .. (isDashEnabled and "ON" or "OFF")
        BtnDash.TextColor3 = isDashEnabled and C_GREEN or C_RED
        BtnDash.UIStroke.Color = isDashEnabled and C_GREEN or C_RED
        if isDashEnabled then
            dashInputConn = UserInputService.InputBegan:Connect(function(input, gpe) if not gpe and input.KeyCode == dashKey then PerformDash() end end)
            table.insert(Connections, dashInputConn)
        else
            if dashInputConn then dashInputConn:Disconnect(); dashInputConn = nil end
        end
    end)

    -- 9. LEDGE GRAB
    local CardLedge = CreateCard("LEDGE GRAB", 135, 9)
    local BtnLedge = Instance.new("TextButton", CardLedge)
    BtnLedge.Text = "LEDGE GRAB: OFF"
    BtnLedge.Size = UDim2.new(0.94, 0, 0, 35)
    BtnLedge.Position = UDim2.new(0.03, 0, 0, 35)
    StyleBtn(BtnLedge, C_RED)
    local LblLedgeThreshold = Instance.new("TextLabel", CardLedge)
    LblLedgeThreshold.Text = "GRAB RANGE: 3.0 studs"
    LblLedgeThreshold.Size = UDim2.new(1, -20, 0, 20)
    LblLedgeThreshold.Position = UDim2.new(0, 15, 0, 80)
    LblLedgeThreshold.BackgroundTransparency = 1
    LblLedgeThreshold.TextColor3 = C_TEXT_DIM
    LblLedgeThreshold.Font = Enum.Font.GothamBold
    LblLedgeThreshold.TextSize = 10
    LblLedgeThreshold.TextXAlignment = Enum.TextXAlignment.Left
    local SldLedgeBg = Instance.new("TextButton", CardLedge)
    SldLedgeBg.Text = ""
    SldLedgeBg.Size = UDim2.new(0.9, 0, 0, 6)
    SldLedgeBg.Position = UDim2.new(0.05, 0, 0, 100)
    SldLedgeBg.BackgroundColor3 = C_SIDE
    SldLedgeBg.AutoButtonColor = false
    Instance.new("UICorner", SldLedgeBg).CornerRadius = UDim.new(0, 3)
    local SldLedgeFill = Instance.new("Frame", SldLedgeBg)
    SldLedgeFill.Size = UDim2.new(0.5, 0, 1, 0)
    SldLedgeFill.BackgroundColor3 = C_ACCENT
    Instance.new("UICorner", SldLedgeFill).CornerRadius = UDim.new(0, 3)
    local ledgeThreshold, isLedgeGrab, ledgeLoop = 3.0, false, nil
    local function UpdateLedgeSlider(input)
        local sc = math.clamp((input.Position.X - SldLedgeBg.AbsolutePosition.X) / SldLedgeBg.AbsoluteSize.X, 0, 1)
        ledgeThreshold = 1.0 + (sc * 4.0)
        SldLedgeFill.Size = UDim2.new(sc, 0, 1, 0)
        LblLedgeThreshold.Text = string.format("GRAB RANGE: %.1f studs", ledgeThreshold)
    end
    local draggingLedge = false
    SldLedgeBg.MouseButton1Down:Connect(function() draggingLedge = true end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then draggingLedge = false end end)
    UserInputService.InputChanged:Connect(function(i) if draggingLedge and i.UserInputType == Enum.UserInputType.MouseMovement then UpdateLedgeSlider(i) end end)
    BtnLedge.MouseButton1Click:Connect(function()
        isLedgeGrab = not isLedgeGrab
        BtnLedge.Text = "LEDGE GRAB: " .. (isLedgeGrab and "ON" or "OFF")
        BtnLedge.TextColor3 = isLedgeGrab and C_GREEN or C_RED
        BtnLedge.UIStroke.Color = isLedgeGrab and C_GREEN or C_RED
        if isLedgeGrab then
            local isGrabbing = false
            ledgeLoop = RunService.Heartbeat:Connect(function()
                local c, h, r = LocalPlayer.Character, nil, nil
                if c then h = c:FindFirstChild("Humanoid"); r = c:FindFirstChild("HumanoidRootPart") end
                if not h or not r then return end
                local state, vel = h:GetState(), r.AssemblyLinearVelocity
                if state == Enum.HumanoidStateType.Freefall and vel.Y < 0 and not isGrabbing then
                    local params = RaycastParams.new()
                    params.FilterDescendantsInstances = {c}
                    params.FilterType = Enum.RaycastFilterType.Exclude
                    local lookDir = r.CFrame.LookVector
                    local ray = workspace:Raycast(r.Position + Vector3.new(0, 1, 0), lookDir * ledgeThreshold, params)
                    if ray and ray.Instance then
                        local ledgeRay = workspace:Raycast(ray.Position + Vector3.new(0, 2, 0), Vector3.new(0, -1, 0), params)
                        if ledgeRay and ledgeRay.Instance and ledgeRay.Instance.CanCollide then
                            isGrabbing = true
                            local targetPos = ledgeRay.Position + Vector3.new(0, 3, 0) + (lookDir * 1)
                            local targetCF = CFrame.new(targetPos) * (r.CFrame - r.CFrame.Position)
                            for i = 1, 10 do
                                if not r or not r.Parent then break end
                                r.CFrame = r.CFrame:Lerp(targetCF, 0.3)
                                r.AssemblyLinearVelocity = Vector3.new(vel.X * 0.5, 5, vel.Z * 0.5)
                                task.wait()
                            end
                            task.wait(0.5)
                            isGrabbing = false
            ...
            table.insert(Connections, ledgeLoop)
        else
            if ledgeLoop then ledgeLoop:Disconnect(); ledgeLoop = nil end
        end
    end)
