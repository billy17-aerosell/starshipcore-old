local function SetupHelperUI(PageHelper, UI, Connections, Config, LocalPlayer, UIHandlers, ShowConfirm, RegisterTheme)
    ...
    -- Speed
    local WSVal=Instance.new("TextBox",CardChar); WSVal.Text=""; WSVal.PlaceholderText="16"; WSVal.Size=UDim2.new(0.15,0,0,35); WSVal.Position=UDim2.new(0.82,0,0,35); WSVal.BackgroundColor3=C_SIDE; WSVal.TextColor3=C_TEXT; WSVal.Font=Enum.Font.Gotham; Instance.new("UICorner",WSVal).CornerRadius=UDim.new(0,6)
    local SldWS=Instance.new("TextButton",CardChar); SldWS.Text=""; SldWS.Size=UDim2.new(0.55,0,0,6); SldWS.Position=UDim2.new(0.25,0,0,50); SldWS.BackgroundColor3=C_SIDE; SldWS.AutoButtonColor=false; Instance.new("UICorner",SldWS)
    local FillWS=Instance.new("Frame",SldWS); FillWS.Size=UDim2.new(0,0,1,0); FillWS.BackgroundColor3=C_ACCENT; Instance.new("UICorner",FillWS)
    local BtnTogWS=Instance.new("TextButton",CardChar); BtnTogWS.Text="SPEED: OFF"; BtnTogWS.Size=UDim2.new(0.2,0,0,35); BtnTogWS.Position=UDim2.new(0.03,0,0,35); StyleBtn(BtnTogWS, C_RED)
    
    local isWSEnabled,wsCon,defWS,tgtWS=false,nil,16,16
    local function UpdateWSVisual(v) v=math.clamp(v,0,300); local p=v/300; FillWS.Size=UDim2.new(p,0,1,0); WSVal.Text=tostring(math.floor(v)) end
    local function ApplySpeed(v) local c=LocalPlayer.Character; local h=c and c:FindFirstChild("Humanoid"); if h then if math.abs(h.WalkSpeed-v)>5 then TweenService:Create(h,TweenInfo.new(0.5),{WalkSpeed=v}):Play() else h.WalkSpeed=v end end end
    local function SetWS(v,up) tgtWS=v; UpdateWSVisual(v); if up and isWSEnabled then ApplySpeed(v) end end
    local function ToggleSpeed()
        local c=LocalPlayer.Character; local h=c and c:FindFirstChild("Humanoid")
        if not isWSEnabled then 
            isWSEnabled=true; BtnTogWS.Text="SPEED: ON"; BtnTogWS.TextColor3=C_GREEN; BtnTogWS.UIStroke.Color=C_GREEN
            if h then defWS=h.WalkSpeed end; SetWS(tgtWS,true)
            if wsCon then wsCon:Disconnect() end
            wsCon=RunService.Heartbeat:Connect(function() local c=LocalPlayer.Character; local h=c and c:FindFirstChild("Humanoid"); if h and math.abs(h.WalkSpeed-tgtWS)>1 then h.WalkSpeed=tgtWS end end)
            table.insert(Connections,wsCon) 
        else 
            isWSEnabled=false; BtnTogWS.Text="SPEED: OFF"; BtnTogWS.TextColor3=C_RED; BtnTogWS.UIStroke.Color=C_RED
            if wsCon then wsCon:Disconnect(); wsCon=nil end
            ApplySpeed(defWS); UpdateWSVisual(tgtWS) 
        end 
    end
    BtnTogWS.MouseButton1Click:Connect(ToggleSpeed)
    local dragWS=false; SldWS.MouseButton1Down:Connect(function() dragWS=true end); UserInputService.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dragWS=false end end); UserInputService.InputChanged:Connect(function(i) if dragWS and i.UserInputType==Enum.UserInputType.MouseMovement then local sc=math.clamp((i.Position.X-SldWS.AbsolutePosition.X)/SldWS.AbsoluteSize.X,0,1); local n=math.floor(sc*300); if isWSEnabled then SetWS(n,true) else UpdateWSVisual(n) end end end)
    WSVal.FocusLost:Connect(function() local v=tonumber(WSVal.Text); if v then if isWSEnabled then SetWS(v,true) else UpdateWSVisual(v) end else WSVal.Text="16" end end)
    
    -- Jump
    local JPVal=Instance.new("TextBox",CardChar); JPVal.Text=""; JPVal.PlaceholderText="50"; JPVal.Size=UDim2.new(0.15,0,0,35); JPVal.Position=UDim2.new(0.82,0,0,75); JPVal.BackgroundColor3=C_SIDE; JPVal.TextColor3=C_TEXT; JPVal.Font=Enum.Font.Gotham; Instance.new("UICorner",JPVal).CornerRadius=UDim.new(0,6)
    local SldJP=Instance.new("TextButton",CardChar); SldJP.Text=""; SldJP.Size=UDim2.new(0.55,0,0,6); SldJP.Position=UDim2.new(0.25,0,0,90); SldJP.BackgroundColor3=C_SIDE; SldJP.AutoButtonColor=false; Instance.new("UICorner",SldJP)
    local FillJP=Instance.new("Frame",SldJP); FillJP.Size=UDim2.new(0,0,1,0); FillJP.BackgroundColor3=C_ACCENT; Instance.new("UICorner",FillJP)
    local BtnTogJP=Instance.new("TextButton",CardChar); BtnTogJP.Text="JUMP: OFF"; BtnTogJP.Size=UDim2.new(0.2,0,0,35); BtnTogJP.Position=UDim2.new(0.03,0,0,75); StyleBtn(BtnTogJP, C_RED)
    local isJPEnabled,jpCon,defJP,tgtJP=false,nil,50,50
    local function UpdateJPVisual(v) v=math.clamp(v,0,500); local p=v/500; FillJP.Size=UDim2.new(p,0,1,0); JPVal.Text=tostring(math.floor(v)) end
    local function ApplyJump(v) local c=LocalPlayer.Character; local h=c and c:FindFirstChild("Humanoid"); if h then h.UseJumpPower=true; h.JumpPower=v end end
    local function SetJP(v,up) tgtJP=v; UpdateJPVisual(v); if up and isJPEnabled then ApplyJump(v) end end
    local function ToggleJump()
        local c=LocalPlayer.Character; local h=c and c:FindFirstChild("Humanoid")
        if not isJPEnabled then 
            isJPEnabled=true; BtnTogJP.Text="JUMP: ON"; BtnTogJP.TextColor3=C_GREEN; BtnTogJP.UIStroke.Color=C_GREEN
            if h then h.UseJumpPower=true; defJP=h.JumpPower end; SetJP(tgtJP,true)
            if jpCon then jpCon:Disconnect() end
            jpCon=RunService.Heartbeat:Connect(function() local c=LocalPlayer.Character; local h=c and c:FindFirstChild("Humanoid"); if h and math.abs(h.JumpPower-tgtJP)>1 then h.UseJumpPower=true; h.JumpPower=tgtJP end end)
            table.insert(Connections,jpCon) 
        else 
            isJPEnabled=false; BtnTogJP.Text="JUMP: OFF"; BtnTogJP.TextColor3=C_RED; BtnTogJP.UIStroke.Color=C_RED
            if jpCon then jpCon:Disconnect(); jpCon=nil end
            ApplyJump(defJP); UpdateJPVisual(tgtJP) 
        end 
    end
    BtnTogJP.MouseButton1Click:Connect(ToggleJump)
    local dragJP=false; SldJP.MouseButton1Down:Connect(function() dragJP=true end); UserInputService.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 then dragJP=false end end); UserInputService.InputChanged:Connect(function(i) if dragJP and i.UserInputType==Enum.UserInputType.MouseMovement then local sc=math.clamp((i.Position.X-SldJP.AbsolutePosition.X)/SldJP.AbsoluteSize.X,0,1); local n=math.floor(sc*500); if isJPEnabled then SetJP(n,true) else UpdateJPVisual(n); tgtJP=n end end end)
    JPVal.FocusLost:Connect(function() local v=tonumber(JPVal.Text); if v then if isJPEnabled then SetJP(v,true) else UpdateJPVisual(v); tgtJP=v end else JPVal.Text="50" end end)
    
    -- Inf Jump
    local BtnInfJump=Instance.new("TextButton",CardChar); BtnInfJump.Text="INFINITE JUMP: OFF"; BtnInfJump.Size=UDim2.new(0.94,0,0,30); BtnInfJump.Position=UDim2.new(0.03,0,0,115); StyleBtn(BtnInfJump, C_TEXT_DIM)
    ...
    local function ToggleInfJump()
        ...
        if isInfJump then 
            infJumpCon=UserInputService.JumpRequest:Connect(function() local c=LocalPlayer.Character; if c then local h=c:FindFirstChildOfClass("Humanoid"); if h then h:ChangeState("Jumping") end end end)
            table.insert(Connections,infJumpCon) 
        else 
            if infJumpCon then infJumpCon:Disconnect(); infJumpCon=nil end 
        end 
    end
    BtnInfJump.MouseButton1Click:Connect(ToggleInfJump)

    -- Fly
    local BtnFly=Instance.new("TextButton",CardChar); BtnFly.Text="Fly: OFF"; BtnFly.Size=UDim2.new(0.45,0,0,35); BtnFly.Position=UDim2.new(0.03,0,0,150); StyleBtn(BtnFly, C_TEXT_DIM)
    local InpFlySpd=Instance.new("TextBox",CardChar); InpFlySpd.PlaceholderText="Spd"; InpFlySpd.Text="50"; InpFlySpd.Size=UDim2.new(0.45,0,0,35); InpFlySpd.Position=UDim2.new(0.52,0,0,150); InpFlySpd.BackgroundColor3=C_SIDE; InpFlySpd.TextColor3=C_TEXT; InpFlySpd.Font=Enum.Font.Gotham; Instance.new("UICorner",InpFlySpd).CornerRadius=UDim.new(0,6)
    local FlySpeed,isFlying=50,false; InpFlySpd.FocusLost:Connect(function() FlySpeed=tonumber(InpFlySpd.Text) or 50 end)
    
    local function StopFly() 
        isFlying=false; BtnFly.Text="Fly: OFF"; BtnFly.TextColor3=C_TEXT_DIM; BtnFly.UIStroke.Color=C_TEXT_DIM
        local c=LocalPlayer.Character; 
        if c then 
            local r=c:FindFirstChild("HumanoidRootPart"); 
            if r then 
                for _,x in pairs(r:GetChildren()) do if x.Name=="AmethystFlyVel" or x.Name=="AmethystFlyGyro" then x:Destroy() end end 
            end 
            local h=c:FindFirstChild("Humanoid"); if h then h.PlatformStand=false end 
        end
    ...
    local function StartFly() 
        ...
        isFlying=true; BtnFly.Text="Fly: ON"; BtnFly.TextColor3=C_GREEN; BtnFly.UIStroke.Color=C_GREEN; h.PlatformStand=true; 
        local bv=Instance.new("BodyVelocity",r); bv.Name="AmethystFlyVel"; bv.MaxForce=Vector3.new(math.huge,math.huge,math.huge); bv.Velocity=Vector3.new(0,0,0); 
        local bg=Instance.new("BodyGyro",r); bg.Name="AmethystFlyGyro"; bg.MaxTorque=Vector3.new(math.huge,math.huge,math.huge); bg.P=10000; bg.D=1000; 
        
        local flyLoop = game:GetService("RunService").RenderStepped:Connect(function() 
            if not isFlying or not c.Parent then StopFly() return end; 
            local cam=workspace.CurrentCamera; local m=Vector3.new(0,0,0); local k=UserInputService:GetKeysPressed(); 
            local W,A,S,D=false,false,false,false; 
            for _,key in pairs(k) do 
                if key.KeyCode==Enum.KeyCode.W then W=true 
                elseif key.KeyCode==Enum.KeyCode.A then A=true 
                elseif key.KeyCode==Enum.KeyCode.S then S=true 
                elseif key.KeyCode==Enum.KeyCode.D then D=true end 
            end; 
            if W then m=m+cam.CFrame.LookVector end; 
            if S then m=m-cam.CFrame.LookVector end; 
            if A then m=m-cam.CFrame.RightVector end; 
            if D then m=m+cam.CFrame.RightVector end; 
            bg.CFrame=cam.CFrame; bv.Velocity=m*FlySpeed 
        end)
        
        Connections.FlyLoop = flyLoop 
    end
    
    local function ToggleFly() if isFlying then StopFly() else StartFly() end end
    ...
    do
        ...
        local function ToggleAutoJump()
            ...
            if isAutoJump then
                ...
                if autoJumpLoop then autoJumpLoop:Disconnect(); autoJumpLoop = nil end
            end
        end

        BtnAutoJump.MouseButton1Click:Connect(ToggleAutoJump)
        ...
        SldPowerBg.MouseButton1Down:Connect(function() draggingPower = true end)
        UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then draggingPower = false end end)
        ...
        local function ToggleLongJump()
            ...
            if isLongJump then
                ...
                longJumpLoop = RunService.Heartbeat:Connect(function()
                     ...
                     if h and r then
                         ...
                         if state == Enum.HumanoidStateType.Jumping and lastState ~= Enum.HumanoidStateType.Jumping then
                             ...
                             r.AssemblyLinearVelocity = Vector3.new(dir.X * (Config.LongJumpPower or 50), r.AssemblyLinearVelocity.Y, dir.Z * (Config.LongJumpPower or 50))
                         end
                         lastState = state
                     end
                end)
                ...
                if longJumpLoop then longJumpLoop:Disconnect(); longJumpLoop = nil end
            end
        end

        BtnLongJump.MouseButton1Click:Connect(ToggleLongJump)
        ...
        local function ToggleAirLock()
            ...
            if isAirLock then
                ...
                if airLockLoop then airLockLoop:Disconnect(); airLockLoop = nil end
                local c = LocalPlayer.Character
                local h = c and c:FindFirstChild("Humanoid")
                if h then h.AutoRotate = true end
            end
        end
        BtnAirLock.MouseButton1Click:Connect(ToggleAirLock)
    end
    ...
    local function CreateMomentumHUD()
        if momentumHUD then return end
    ...
    local function DestroyMomentumHUD()
        if momentumHUD then momentumHUD:Destroy(); momentumHUD = nil end
    end

    local function UpdateMomentumHUD(currentSpeed, locked)
        if not momentumHUD then return end
    ...
    local function ToggleMomentum()
        ...
        if isMomentum then
                ...
                if h and r and h.MoveDirection.Magnitude > 0.1 then
                    ...
                    if horizontalSpeed < lockedSpeed then
                        ...
                        r.AssemblyLinearVelocity = Vector3.new(
                            moveDir.X * lockedSpeed,
                            vel.Y,
                            moveDir.Z * lockedSpeed
                        )
            ...
            if momentumLoop then momentumLoop:Disconnect(); momentumLoop = nil end
            lockedSpeed = nil
            DestroyMomentumHUD()
        end
    end

    BtnMomentum.MouseButton1Click:Connect(ToggleMomentum)
    ...
    SldBg.MouseButton1Down:Connect(function() dragging = true end)
    ...
    BtnSlip.MouseButton1Click:Connect(function()
        ...
        if isSlipOn then
            ...
            if slipLoop then slipLoop:Disconnect(); slipLoop = nil end
            lastSafeY = nil
            lastGroundPart = nil
    ...
    -- 3. CLIMB ASSIST
    local CardClimb = CreateCard("CLIMB ASSIST", 115, 3)
    
    local BtnFastClimb = Instance.new("TextButton", CardClimb)
    BtnFastClimb.Text = "FAST CLIMB: OFF"
    BtnFastClimb.Size = UDim2.new(0.94, 0, 0, 35)
    BtnFastClimb.Position = UDim2.new(0.03, 0, 0, 35)
    StyleBtn(BtnFastClimb, C_TEXT_DIM)

    local isFastClimb, fastClimbLoop = false, nil
    local function ToggleFastClimb()
        isFastClimb = not isFastClimb
        BtnFastClimb.Text = "FAST CLIMB: " .. (isFastClimb and "ON" or "OFF")
        BtnFastClimb.TextColor3 = isFastClimb and C_GREEN or C_TEXT_DIM
        BtnFastClimb.UIStroke.Color = isFastClimb and C_GREEN or C_TEXT_DIM
        
        if isFastClimb then
            fastClimbLoop = RunService.Heartbeat:Connect(function()
                local c = LocalPlayer.Character
                local h = c and c:FindFirstChild("Humanoid")
                local r = c and c:FindFirstChild("HumanoidRootPart")
                if h and r and h:GetState() == Enum.HumanoidStateType.Climbing then
                    r.AssemblyLinearVelocity = Vector3.new(r.AssemblyLinearVelocity.X, 20, r.AssemblyLinearVelocity.Z)
                end
            end)
            table.insert(Connections, fastClimbLoop)
        else
            if fastClimbLoop then fastClimbLoop:Disconnect(); fastClimbLoop = nil end
        end
    end
    BtnFastClimb.MouseButton1Click:Connect(ToggleFastClimb)

    -- Ladder Assist (gabungan Platform Magnet + Platform Stick)
    local BtnLadderAssist = Instance.new("TextButton", CardClimb)
    BtnLadderAssist.Text = "LADDER ASSIST: OFF"
    BtnLadderAssist.Size = UDim2.new(0.94, 0, 0, 35)
    BtnLadderAssist.Position = UDim2.new(0.03, 0, 0, 75)
    StyleBtn(BtnLadderAssist, C_TEXT_DIM)

    local isLadderAssist, ladderAssistLoop = false, nil
    local function ToggleLadderAssist()
        isLadderAssist = not isLadderAssist
        BtnLadderAssist.Text = "LADDER ASSIST: " .. (isLadderAssist and "ON" or "OFF")
        BtnLadderAssist.TextColor3 = isLadderAssist and C_GREEN or C_TEXT_DIM
        BtnLadderAssist.UIStroke.Color = isLadderAssist and C_GREEN or C_TEXT_DIM
        
        if isLadderAssist then
            ladderAssistLoop = RunService.Heartbeat:Connect(function()
                local c = LocalPlayer.Character
                local r = c and c:FindFirstChild("HumanoidRootPart")
                local h = c and c:FindFirstChild("Humanoid")
                ...
                if r and h then
                    ...
                    if state == Enum.HumanoidStateType.Freefall or state == Enum.HumanoidStateType.Jumping then
                        -- Check ladder in front (Stick behavior)
                        local rayParams = RaycastParams.new()
                        rayParams.FilterDescendantsInstances = {c}
                        rayParams.FilterType = Enum.RaycastFilterType.Exclude
                        local frontRay = workspace:Raycast(r.Position, r.CFrame.LookVector * 2, rayParams)
                        
                        if frontRay and frontRay.Instance then
                            local p = frontRay.Instance
                            if p:IsA("TrussPart") or p.Name:lower():find("ladder") or p.Name:lower():find("truss") then
                                r.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
                                return
                            end
                        end
                        
                        -- Magnet behavior - pull towards nearby ladder
                        local range = 15
                        ...
                        for _, p in pairs(parts) do
                            if (p:IsA("TrussPart") or p.Name:lower():find("ladder") or p.Name:lower():find("truss")) and p.CanCollide then
                                if r.Position.Y > (p.Position.Y + 1) then
                                    local dist = (Vector3.new(p.Position.X, 0, p.Position.Z) - Vector3.new(r.Position.X, 0, r.Position.Z)).Magnitude
                                    if dist < minDist then minDist = dist; closestPart = p end
                                end
                            end
                        end
                        if closestPart then
                            local pPos = closestPart.Position; local rPos = r.Position
                            if rPos.Y > (pPos.Y + 0.5) then
                                local targetPos = Vector3.new(pPos.X, rPos.Y, pPos.Z)
                                local alpha = 0.15
                                local newPos = rPos:Lerp(targetPos, alpha)
                                r.CFrame = CFrame.new(newPos.X, rPos.Y, newPos.Z) * r.CFrame.Rotation
                            elseif rPos.Y > (pPos.Y - 5.0) then
                                local dirToCenter = (Vector3.new(pPos.X, 0, pPos.Z) - Vector3.new(rPos.X, 0, rPos.Z)).Unit
                                local distToCenter = (Vector3.new(pPos.X, 0, pPos.Z) - Vector3.new(rPos.X, 0, rPos.Z)).Magnitude
                                if distToCenter < 8 then
                                    r.AssemblyLinearVelocity = Vector3.new(r.AssemblyLinearVelocity.X, 45, r.AssemblyLinearVelocity.Z)
                                    r.CFrame = r.CFrame + (dirToCenter * 0.8) 
                                end
                            end
                        end
                    end
            ...
            table.insert(Connections, ladderAssistLoop)
        else
            if ladderAssistLoop then ladderAssistLoop:Disconnect(); ladderAssistLoop = nil end
        end
    end
    BtnLadderAssist.MouseButton1Click:Connect(ToggleLadderAssist)
    ...
    local function CreateHighlight(part, color)
        if highlightedParts[part] then return end
    ...
    local function ToggleRealESP()
        ...
        if isRealESP then
                ...
                for _, p in pairs(workspace:GetDescendants()) do
                    if p:IsA("BasePart") then
                        ...
                        if shouldInclude then
                            ...
                            local isCharPart = p:FindFirstAncestorOfClass("Model") and p:FindFirstAncestorOfClass("Model"):FindFirstChild("Humanoid")
                            if not isCharPart and not p:IsDescendantOf(espContainer) then
                                local dist = (p.Position - charPos).Magnitude
                                if dist <= range then
                                    local color
                                    local isLadder = p:IsA("TrussPart") or p.Name:lower():find("ladder") or p.Name:lower():find("truss") or p.Name:lower():find("climb")
            ...
            local c = LocalPlayer.Character
            local r = c and c:FindFirstChild("HumanoidRootPart")
            if r then ScanAllParts(r.Position, 75) end
            
            -- Periodic rescan for new parts entering range (every 0.5 sec)
            local lastScan = 0
            espLoop = RunService.Heartbeat:Connect(function()
                local now = tick()
                if now - lastScan < 0.5 then return end -- Only scan every 0.5 seconds
                lastScan = now
    ...
    BtnRealESP.MouseButton1Click:Connect(ToggleRealESP)
    ...
    local function GetGhostPath()
        ...
        if not isfolder(ghostRoot) then makefolder(ghostRoot) end
    ...
    local sws = Instance.new("UIStroke", BtnGWorkspace); sws.Color = C_ACCENT; sws.Transparency = 0.6; sws.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    ...
    local sl = Instance.new("UIStroke", GWSList); sl.Color = C_ACCENT; sl.Transparency = 0.6
    ...
    local function UpdateGWSList()
        for _, c in pairs(GWSScroll:GetChildren()) do if c:IsA("TextButton") then c:Destroy() end end
        if isfolder(FOLDER_NAME) then
            local folders = listfiles(FOLDER_NAME)
            GWSScroll.CanvasSize = UDim2.new(0, 0, 0, #folders * 27)
    ...
    BtnGWorkspace.MouseButton1Click:Connect(function()
        ...
        if GWSList.Visible then UpdateGWSList() end
    end)
    ...
    RefreshGhostList = function()
        for _, c in pairs(GhostListScroll:GetChildren()) do if c:IsA("TextButton") then c:Destroy() end end
                    ...
                    if s then
                        ...
                        if d.Frames then
                            ...
                            for _, fr in ipairs(d.Frames) do
                                ...
                                for ln, lcf in pairs(fr.Limbs) do frame.Limbs[ln] = TblToCF(lcf) end
    ...
    BtnSaveGhost.MouseButton1Click:Connect(function()
        ...
        if name == "" or #GhostData == 0 then return end
        local saveData = {Frames = {}}
        for _, frame in ipairs(GhostData) do
            local saveFrame = {Time = frame.Time, RootCF = CFToTbl(frame.RootCF), Limbs = {}}
            for limbName, limbCF in pairs(frame.Limbs) do saveFrame.Limbs[limbName] = CFToTbl(limbCF) end
    ...
    local function StopGhost()
        if ghostRecLoop then ghostRecLoop:Disconnect(); ghostRecLoop = nil end
        if ghostPlayLoop then ghostPlayLoop:Disconnect(); ghostPlayLoop = nil end
    ...
    BtnRecordGhost.MouseButton1Click:Connect(function()
        if isGhostRecording then StopGhost() else
            StopGhost()
            StartCountdown(function()
                ...
                local c = LocalPlayer.Character
                local partsToRecord = {}
                if c then for _, p in pairs(c:GetDescendants()) do if p:IsA("BasePart") and p.Name ~= "HumanoidRootPart" then table.insert(partsToRecord, p) end end end
                ...
                ghostRecLoop = RunService.Heartbeat:Connect(function()
                    ...
                    if root then
                        ...
                        for _, p in pairs(partsToRecord) do if p and p.Parent then frameData.Limbs[p.Name] = p.CFrame end end
    ...
    BtnPlayGhost.MouseButton1Click:Connect(function()
        if isGhostPlaying then StopGhost() elseif #GhostData > 0 then
            StopGhost()
            StartCountdown(function()
                isGhostPlaying = true
                ...
                for _, p in pairs(GhostModel:GetDescendants()) do
                    if p:IsA("BasePart") then p.Anchored = true; p.CanCollide = false; p.Transparency = 0.6; p.Color = C_ACCENT; p.Material = Enum.Material.ForceField
                    elseif p:IsA("Script") or p:IsA("LocalScript") or p:IsA("Sound") then p:Destroy()
                    elseif p:IsA("Humanoid") then p.DisplayDistanceType = Enum.HumanoidDisplayDistanceType.None end
                end
                local playStart = os.clock()
                local index = 1
                ghostPlayLoop = RunService.Heartbeat:Connect(function()
                    if not GhostModel or not GhostModel.Parent then StopGhost(); return end
                    local timeElapsed = os.clock() - playStart
                    while index < #GhostData and GhostData[index+1].Time <= timeElapsed do index = index + 1 end
                    local frame = GhostData[index]
                    if frame then
                        if GhostModel.PrimaryPart then GhostModel:SetPrimaryPartCFrame(frame.RootCF)
                        elseif GhostModel:FindFirstChild("HumanoidRootPart") then GhostModel.HumanoidRootPart.CFrame = frame.RootCF end
                        for name, cf in pairs(frame.Limbs) do local p = GhostModel:FindFirstChild(name, true) if p and p:IsA("BasePart") then p.CFrame = cf end end
                    end
                    if index >= #GhostData then playStart = os.clock(); index = 1 end
                end)
    ...
    BtnClearGhost.MouseButton1Click:Connect(function() StopGhost(); GhostData = {}; LblGhostStatus.Text = "STATUS: CLEARED"; BtnPlayGhost.TextColor3 = C_TEXT_DIM; BtnPlayGhost.UIStroke.Color = C_TEXT_DIM end)
    ...
    UIHandlers.CleanupTools = function()
        ...
        if slopeLoop then slopeLoop:Disconnect(); slopeLoop = nil end
        if slopeHUD then slopeHUD:Destroy(); slopeHUD = nil end
        if slipLoop then slipLoop:Disconnect(); slipLoop = nil end
        if fastClimbLoop then fastClimbLoop:Disconnect(); fastClimbLoop = nil end
        if magnetLoop then magnetLoop:Disconnect(); magnetLoop = nil end
        if stickLoop then stickLoop:Disconnect(); stickLoop = nil end
        if espLoop then espLoop:Disconnect(); espLoop = nil end
        if ghostRecLoop then ghostRecLoop:Disconnect(); ghostRecLoop = nil end
        if ghostPlayLoop then ghostPlayLoop:Disconnect(); ghostPlayLoop = nil end
        if GhostModel then GhostModel:Destroy(); GhostModel = nil end
        if espContainer then espContainer:ClearAllChildren() end
        RestorePlatforms()
    end
end
