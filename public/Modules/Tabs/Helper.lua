local function SetupHelperUI(PageHelper, UI, Connections, Config, LocalPlayer, UIHandlers, ShowConfirm, RegisterTheme)
	...
	local function UpdateStrafeSlider(input)
		...
		local sc = math.clamp(rx / SldStrafeBg.AbsoluteSize.X, 0, 1)
	...
	SldStrafeBg.MouseButton1Down:Connect(function()
		draggingStrafe = true
	end)
	UserInputService.InputEnded:Connect(function(i)
		if i.UserInputType == Enum.UserInputType.MouseButton1 then
			draggingStrafe = false
		end
	end)
	UserInputService.InputChanged:Connect(function(i)
		if draggingStrafe and i.UserInputType == Enum.UserInputType.MouseMovement then
			UpdateStrafeSlider(i)
		end
	end)
		...
		if isStrafing then
			strafeLoop = RunService.Heartbeat:Connect(function()
				...
				if
					h
					and r
					and (h:GetState() == Enum.HumanoidStateType.Jumping or h:GetState() == Enum.HumanoidStateType.Freefall)
					and h.MoveDirection.Magnitude > 0.1
				then
					local vel = r.AssemblyLinearVelocity
	...
	local dashPower, dashCooldown, dashKey, isDashEnabled, lastDash, isSettingDashKey =
		50, 0.5, Enum.KeyCode.Q, false, 0, false
	local dashInputConn = nil
	local function UpdateDashPowerSlider(input)
		local sc =
			math.clamp((input.Position.X - SldDashPowerBg.AbsolutePosition.X) / SldDashPowerBg.AbsoluteSize.X, 0, 1)
		dashPower = 20 + (sc * 80)
	...
	SldDashPowerBg.MouseButton1Down:Connect(function()
		draggingDashPower = true
	end)
	SldDashCDBg.MouseButton1Down:Connect(function()
		draggingDashCD = true
	end)
	UserInputService.InputEnded:Connect(function(i)
		if i.UserInputType == Enum.UserInputType.MouseButton1 then
			draggingDashPower, draggingDashCD = false, false
		end
	end)
	UserInputService.InputChanged:Connect(function(i)
		if i.UserInputType == Enum.UserInputType.MouseMovement then
			if draggingDashPower then
				UpdateDashPowerSlider(i)
			end
			if draggingDashCD then
				UpdateDashCDSlider(i)
			end
		end
	end)
	BtnSetDashKey.MouseButton1Click:Connect(function()
		if isSettingDashKey then
			return
		end
	...
	local function PerformDash()
		...
		if not r or tick() - lastDash < dashCooldown then
			return
		end
		lastDash = tick()
		local camCF = workspace.CurrentCamera.CFrame
		...
		if inputX ~= 0 or inputZ ~= 0 then
			dashDir = (camCF.LookVector * inputZ + camCF.RightVector * inputX).Unit
		end
	...
	BtnDash.MouseButton1Click:Connect(function()
		...
		if isDashEnabled then
			dashInputConn = UserInputService.InputBegan:Connect(function(input, gpe)
				if not gpe and input.KeyCode == dashKey then
					PerformDash()
				end
			end)
			table.insert(Connections, dashInputConn)
		else
			if dashInputConn then
				dashInputConn:Disconnect()
				dashInputConn = nil
			end
		end
	end)
	...
	SldLedgeBg.MouseButton1Down:Connect(function()
		draggingLedge = true
	end)
	UserInputService.InputEnded:Connect(function(i)
		if i.UserInputType == Enum.UserInputType.MouseButton1 then
	...
	UserInputService.InputChanged:Connect(function(i)
		if draggingLedge and i.UserInputType == Enum.UserInputType.MouseMovement then
			UpdateLedgeSlider(i)
		end
	end)
			...
			ledgeLoop = RunService.Heartbeat:Connect(function()
				...
				if state == Enum.HumanoidStateType.Freefall and vel.Y < 0 and not isGrabbing then
					...
					if ray and ray.Instance then
						local ledgeRay =
							workspace:Raycast(ray.Position + Vector3.new(0, 2, 0), Vector3.new(0, -1, 0), params)
