local function SetupToolsUI(PageTools, UI, Connections, Config, LocalPlayer, UIHandlers, RegisterTheme)
    ...
    local ToolsScroll = Instance.new("ScrollingFrame", PageTools); ToolsScroll.Size=UDim2.new(1,0,1,0); ToolsScroll.BackgroundTransparency=1; ToolsScroll.BorderSizePixel=0; ToolsScroll.ScrollBarThickness=4; ToolsScroll.ScrollBarImageColor3=C_ACCENT; ToolsScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y; ToolsScroll.CanvasSize=UDim2.new(0,0,0,0)
    RegisterTheme(ToolsScroll, "ScrollBarImageColor3", "Accent")
    local ToolsLayout = Instance.new("UIListLayout", ToolsScroll); ToolsLayout.Padding=UDim.new(0,15); ToolsLayout.SortOrder=Enum.SortOrder.LayoutOrder; ToolsLayout.HorizontalAlignment=Enum.HorizontalAlignment.Center
    
    local function CreateCard(t,h,o) 
        local c=Instance.new("Frame",ToolsScroll); c.Size=UDim2.new(0.96,0,0,h); c.BackgroundColor3=C_ITEM; c.LayoutOrder=o; 
        Instance.new("UICorner",c).CornerRadius=UDim.new(0,12); 
        local s=Instance.new("UIStroke",c); s.Color=C_ACCENT; s.Transparency=0.8; s.Thickness=1
        local l=Instance.new("TextLabel",c); l.Text=t; l.Size=UDim2.new(1,-20,0,30); l.Position=UDim2.new(0,15,0,0); l.BackgroundTransparency=1; l.TextColor3=C_TEXT_DIM; l.Font=Enum.Font.GothamBold; l.TextSize=11; l.TextXAlignment=Enum.TextXAlignment.Left; 
    ...
    local function CreateSpeedDisplay()
        if speedScreenGui then speedScreenGui:Destroy() end
    ...
    local function ToggleSpeedCheck()
        ...
        if isSpeedCheck then
            ...
            if speedScreenGui then speedScreenGui:Destroy(); speedScreenGui = nil; speedGui = nil end
        end
    end
    ...
    BtnEmotes.MouseButton1Click:Connect(function()
        if UIHandlers.ToggleEmoteWindow then UIHandlers.ToggleEmoteWindow() end
    end)
    ...
    local function ToggleSL(forceEnable)
        ...
        if isSL then
            ...
            if slLoop then slLoop:Disconnect(); slLoop = nil end
            local c = LocalPlayer.Character
            local hum = c and c:FindFirstChild("Humanoid")
    ...
    local function CheckForAdmin(player)
        ...
        elseif game.CreatorType == Enum.CreatorType.Group then
            ...
            if s2 and role then
                ...
                if lowerRole:find("admin") or lowerRole:find("mod") or lowerRole:find("staff") or lowerRole:find("dev") or lowerRole:find("owner") then
                    isAdmin = true
                end
            end
        end
        ...
        if isAdmin then
            LocalPlayer:Kick("⚠️ Safety Triggered: Admin (" .. player.Name .. ") detected. Exiting to protect your account.")
        end
    end
    ...
    local function ToggleBypassAdmin(forceEnable)
        ...
        if isAntiAdmin then
            for _, p in ipairs(Players:GetPlayers()) do CheckForAdmin(p) end
    ...
    SldTime.MouseButton1Down:Connect(function() dragTime = true end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then dragTime = false end end)
    ...
    local function NotifyAnim(text)
        ...
        if UI and UI.ShowToast then UI.ShowToast("Animation System", text, "info", 2) end
    end
    ...
    if isfile(ANIM_FILE) then
        local success, result = pcall(function() return HttpService:JSONDecode(readfile(ANIM_FILE)) end)
        if success and result then
            for k, v in pairs(result) do
                if AnimDB[k] then
                    for name, id in pairs(v) do AnimDB[k][name] = id end
                else
                    AnimDB[k] = v
                end
            end
    ...
    local function CaptureOriginalAnims()
        ...
        if not OriginalAnims.Walk then OriginalAnims.Walk = getId(Animate:FindFirstChild("walk") and Animate.walk:FindFirstChild("WalkAnim")) end
        if not OriginalAnims.Run then OriginalAnims.Run = getId(Animate:FindFirstChild("run") and Animate.run:FindFirstChild("RunAnim")) end
        if not OriginalAnims.Jump then OriginalAnims.Jump = getId(Animate:FindFirstChild("jump") and Animate.jump:FindFirstChild("JumpAnim")) end
        if not OriginalAnims.Fall then OriginalAnims.Fall = getId(Animate:FindFirstChild("fall") and Animate.fall:FindFirstChild("FallAnim")) end
        if not OriginalAnims.Climb then OriginalAnims.Climb = getId(Animate:FindFirstChild("climb") and Animate.climb:FindFirstChild("ClimbAnim")) end
        if not OriginalAnims.Swim then OriginalAnims.Swim = getId(Animate:FindFirstChild("swim") and Animate.swim:FindFirstChild("Swim")) end
        if not OriginalAnims.SwimIdle then OriginalAnims.SwimIdle = getId(Animate:FindFirstChild("swimidle") and Animate.swimidle:FindFirstChild("SwimIdle")) end
    end
    ...
    local function SetAnimation(animType, animId)
        ...
        local function freeze()
            ...
            for _, p in pairs(Char:GetDescendants()) do if p:IsA("BasePart") and not p.Anchored then p.Anchored = true end end
        end
        local function unfreeze()
            local h = Char:FindFirstChild("Humanoid")
            if h then h.PlatformStand = false end
            for _, p in pairs(Char:GetDescendants()) do if p:IsA("BasePart") and p.Anchored then p.Anchored = false end end
        end
        ...
        local success, err = pcall(function()
            ...
            elseif animType == "Walk" then Animate.walk.WalkAnim.AnimationId = formatId(animId)
            elseif animType == "Run" then Animate.run.RunAnim.AnimationId = formatId(animId)
            elseif animType == "Jump" then Animate.jump.JumpAnim.AnimationId = formatId(animId)
            elseif animType == "Fall" then Animate.fall.FallAnim.AnimationId = formatId(animId)
            elseif animType == "Swim" and Animate:FindFirstChild("swim") then Animate.swim.Swim.AnimationId = formatId(animId)
            elseif animType == "SwimIdle" and Animate:FindFirstChild("swimidle") then Animate.swimidle.SwimIdle.AnimationId = formatId(animId)
            elseif animType == "Climb" then Animate.climb.ClimbAnim.AnimationId = formatId(animId)
            end
        end)
        ...
        local h = Char:FindFirstChild("Humanoid")
        if h then h:ChangeState(Enum.HumanoidStateType.Landed) end -- Force refresh
        task.wait(0.1)
        unfreeze()
    ...
    local function RefreshAnimList()
        for _, c in pairs(AnimScroll:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end
        local filter = InpSearch.Text:lower()
        local list = AnimDB[CurrentAnimType] or {}
        local sorted = {}
        for name, id in pairs(list) do table.insert(sorted, {Name=name, Id=id}) end
        ...
        for _, item in ipairs(sorted) do
            if filter == "" or item.Name:lower():find(filter) then
                ...
                b.MouseButton1Click:Connect(function() SetAnimation(CurrentAnimType, item.Id) end)
    ...
    BtnAddAnim.MouseButton1Click:Connect(function()
        ...
        if not AnimDB[CurrentAnimType] then AnimDB[CurrentAnimType] = {} end
    ...
    local SBrowser = Instance.new("Frame", ScreenGui); SBrowser.Name="SBrowser"; SBrowser.Size=UDim2.new(0,500,0,350); SBrowser.Position=UDim2.new(0.5,-250,0.5,-175); SBrowser.BackgroundColor3=C_SIDE; SBrowser.Visible=false; SBrowser.ZIndex=100; Instance.new("UICorner",SBrowser).CornerRadius=UDim.new(0,12); local SBS=Instance.new("UIStroke",SBrowser); SBS.Color=C_ACCENT; SBS.Thickness=1
    RegisterTheme(SBrowser, "BackgroundColor3", "Side")
    RegisterTheme(SBS, "Color", "Accent")
    
    local SBTitle = Instance.new("TextLabel", SBrowser); SBTitle.Text="SERVER LIST"; SBTitle.Size=UDim2.new(1,0,0,40); SBTitle.Position=UDim2.new(0,20,0,0); SBTitle.BackgroundTransparency=1; SBTitle.TextColor3=C_TEXT; SBTitle.Font=Enum.Font.GothamBold; SBTitle.TextSize=18; SBTitle.TextXAlignment=Enum.TextXAlignment.Left; SBTitle.ZIndex=101; SBTitle.Active=true
    RegisterTheme(SBTitle, "TextColor3", "Text")
    
    local SBClose = Instance.new("TextButton", SBrowser); SBClose.Text="×"; SBClose.Size=UDim2.new(0,40,0,40); SBClose.Position=UDim2.new(1,-40,0,0); SBClose.BackgroundTransparency=1; SBClose.TextColor3=C_RED; SBClose.TextSize=24; SBClose.ZIndex=101; SBClose.MouseButton1Click:Connect(function() SBrowser.Visible=false end)
    
    local SBList = Instance.new("ScrollingFrame", SBrowser); SBList.Size=UDim2.new(1,-20,1,-60); SBList.Position=UDim2.new(0,10,0,50); SBList.BackgroundColor3=C_MAIN; SBList.BackgroundTransparency=0.5; SBList.BorderSizePixel=0; SBList.ScrollBarThickness=4; SBList.ZIndex=101; Instance.new("UICorner",SBList).CornerRadius=UDim.new(0,6); local SBLay=Instance.new("UIListLayout",SBList); SBLay.Padding=UDim.new(0,5)
    RegisterTheme(SBList, "BackgroundColor3", "Main")
    
    local SBStat = Instance.new("TextLabel", SBrowser); SBStat.Text="Fetching..."; SBStat.Size=UDim2.new(1,0,1,0); SBStat.BackgroundTransparency=1; SBStat.TextColor3=C_TEXT_DIM; SBStat.Visible=false; SBStat.ZIndex=102
    RegisterTheme(SBStat, "TextColor3", "TextDim")
    local dragging, dragInput, dragStart, startPos
    SBTitle.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = true; dragStart = input.Position; startPos = SBrowser.Position end end)
    SBTitle.InputChanged:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseMovement then dragInput = input end end)
    UserInputService.InputChanged:Connect(function(input) if input == dragInput and dragging then local delta = input.Position - dragStart; SBrowser.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale, startPos.Y.Offset + delta.Y) end end)
    UserInputService.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.MouseButton1 then dragging = false end end)
    
    local function RefreshServers() 
        for _,c in pairs(SBList:GetChildren()) do if c:IsA("Frame") then c:Destroy() end end; SBStat.Visible=true; SBStat.Text="Fetching Servers..."; SBList.Visible=false; 
        task.spawn(function() 
            local url="https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?limit=100&sortOrder=Desc"; 
            local s,r=pcall(function() return game:HttpGet(url) end); 
            if s then 
                local d=HttpService:JSONDecode(r); 
                if d and d.data then 
                    if #d.data==0 then SBStat.Text="No servers found."; SBStat.Visible=true; return end; 
                    for i,sv in ipairs(d.data) do 
                        if sv.playing<sv.maxPlayers and sv.id~=game.JobId then 
                            local f=Instance.new("Frame",SBList); f.Size=UDim2.new(1,-6,0,40); f.BackgroundColor3=C_ITEM; f.ZIndex=101; Instance.new("UICorner",f).CornerRadius=UDim.new(0,4); 
                            RegisterTheme(f, "BackgroundColor3", "Item")
                            
                            local inf=Instance.new("TextLabel",f); inf.Text=string.format("%d/%d Players | %d FPS | %d Ping",sv.playing,sv.maxPlayers,sv.fps or 60,sv.ping or 0); inf.Size=UDim2.new(0.7,0,1,0); inf.Position=UDim2.new(0,10,0,0); inf.BackgroundTransparency=1; inf.TextColor3=C_TEXT; inf.TextXAlignment=Enum.TextXAlignment.Left; inf.Font=Enum.Font.GothamSemibold; inf.TextSize=12; inf.ZIndex=101; 
                            RegisterTheme(inf, "TextColor3", "Text")
                            
                            local bj=Instance.new("TextButton",f); bj.Text="JOIN"; bj.Size=UDim2.new(0.25,0,0.8,0); bj.Position=UDim2.new(0.72,0,0.1,0); bj.BackgroundColor3=C_GREEN; bj.TextColor3=Color3.new(1,1,1); bj.Font=Enum.Font.GothamBold; bj.TextSize=11; bj.ZIndex=101; Instance.new("UICorner",bj).CornerRadius=UDim.new(0,4); 
                            bj.MouseButton1Click:Connect(function() game:GetService("TeleportService"):TeleportToPlaceInstance(game.PlaceId,sv.id,LocalPlayer) end) 
                        end 
                    end; 
                    SBList.CanvasSize=UDim2.new(0,0,0,#SBList:GetChildren()*45); SBStat.Visible=false; SBList.Visible=true 
                else 
                    SBStat.Text="Invalid Data."; SBStat.Visible=true 
                end 
            else 
                SBStat.Text="Error: "..tostring(r); SBStat.Visible=true 
            end 
        end) 
    end
    
    BtnBrowser.MouseButton1Click:Connect(function() SBrowser.Visible=true; RefreshServers() end)
    BtnRejoin.MouseButton1Click:Connect(function() game:GetService("TeleportService"):Teleport(game.PlaceId, LocalPlayer) end)
    ...
    SldStrBg.MouseButton1Down:Connect(function() draggingStr = true end)
    UserInputService.InputEnded:Connect(function(i) if i.UserInputType == Enum.UserInputType.MouseButton1 then draggingStr = false end end)
    UserInputService.InputChanged:Connect(function(i) 
    ...
    local function ToggleSpoof()
        ...
        if isSpoofing then
            ...
            local charCon = LocalPlayer.CharacterAdded:Connect(function(char)
                task.wait(1)
                if isSpoofing then SpoofAllNames() end
            end)
    ...
    local randomNames = {"Steve", "Alex", "ProGamer", "Noob123", "Player", "Guest", "Anonymous", "Shadow", "Phoenix", "Dragon", "Ninja", "Master", "Legend", "Hero", "Star"}
